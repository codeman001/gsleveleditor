...
struct poly_vert
{
	...
	index_point<coord_t>	get_index_point() const
	{
		return index_point<coord_t> ( m_v.x, m_v.y );
	}
...
// Return true if edge (e0v0,e0v1) intersects (e1v0,e1v1).
{
	...
	if ( coincident[0][0] && !coincident[1][1] )
	{
		return false;
	}

	if ( coincident[1][0] && !coincident[0][1] )
	{
		return false;
	}

	if ( coincident[0][1] && !coincident[1][0] )
	{
		return false;
	}

	if ( coincident[1][1] && !coincident[0][0] )
	{
		return false;
	}
...
// Return the next ear to be clipped.
{
	...
	if ( m_ear_count > 6 )
	{
		...
		if ( random_range >= MASK_TABLE_SIZE )
		{
			random_range = MASK_TABLE_SIZE - 1;
		}
		...
		// Struct that holds the state of a triangulation.
{
		...
		int	get_estimated_triangle_count() const
		{
			return m_estimated_triangle_count;
		}
