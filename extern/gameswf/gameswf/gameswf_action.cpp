...
namespace gameswf
{
	...
	struct do_action : public execute_tag
	{
		...
		do_action ( int characterID ) : m_buf ( characterID ) {}
	...
	void	do_init_action_loader ( stream *in, int tag_type, movie_definition_sub *m )
	{
		...
		do_action	*da = new do_action ( sprite_character_id );
		da->read ( in );
	...
	action_buffer::action_buffer ( int characterID ) :
							...
							else
							{
								...
								if ( env->top ( 1 ).to_object() )
								{
									...
									if ( clip )
									{
										...
										log_error ( "error: can't find method %s[0x%p, _name='%s'].%s\n",
										            last_varname.c_str(), clip, val.to_string(), method_name.c_str() );
									}

									else
									{
										log_error ( "error: can't find function %s[0x%p].%s\n",
								...
								{
									log_error ( "error: can't find function %s[0x0].%s\n",
