...
namespace gameswf
{
	...
	*/
	...
	// ** dm
	inline Uint32 unpremultiply ( Uint32 c, float inverseAlpha )
	{
		Uint32 result = Uint32 ( float ( c ) * inverseAlpha );
					...
					for ( int j = 0; j < height; j++ )
					{
						...
						for ( int i = 0; i < width; i++ )
						{
							...
							/*	image_row[i * 4 + 0] = r;
								...
								image_row[i * 4 + 2] = b;
								image_row[i * 4 + 3] = a;*/
							float inverseAlpha = 255.0f / a;
							image_row[i * 4 + 0] = unpremultiply ( r, inverseAlpha );
							image_row[i * 4 + 1] = unpremultiply ( g, inverseAlpha );
							image_row[i * 4 + 2] = unpremultiply ( b, inverseAlpha );
							image_row[i * 4 + 3] = a;
						}
					}
				}
	...
	struct remove_object_2 : public execute_tag
	{
		...
		virtual bool is_remove_tag() const
		{
			return true;
		}
	};
