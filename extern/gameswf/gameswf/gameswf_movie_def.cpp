...
namespace gameswf
{
	...
	// ...
	float	movie_def_impl::get_frame_rate() const
	{
		return m_frame_rate;
	}
	void	movie_def_impl::set_frame_rate ( float rate )
	{
		m_frame_rate = rate;
	}
	float	movie_def_impl::get_width_pixels() const
	{
		return ceilf ( TWIPS_TO_PIXELS ( m_frame_size.width() ) );
	}
	float	movie_def_impl::get_height_pixels() const
	{
		return ceilf ( TWIPS_TO_PIXELS ( m_frame_size.height() ) );
	}

	int	movie_def_impl::get_version() const
	{
		return m_version;
	}
	uint32	movie_def_impl::get_file_bytes() const
	{
	...
	int	movie_def_impl::get_bitmap_info_count() const
	{
...
#define CACHE_FILE_VERSION 6
	...
	as_function *movie_def_impl::instanciate_class ( character *ch ) const
	{
		...
		if ( m_symbol_class.get ( symbol_id, &class_name ) )
		{
			...
			ch->set_instance ( ii );
			ch->create_traits ( m_abc, ii );
			// create traits
			// Dreemchest: moved to as_object::create_traits
