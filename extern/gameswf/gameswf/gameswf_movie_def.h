...
namespace gameswf
{
	...
	struct movie_definition_sub : public movie_definition
	{
			...
			virtual bool is ( int class_id ) const
			{
				...
				{
					return movie_definition::is ( class_id );
				}
			}
			...
			virtual bool is_multithread() const
			{
				return false;
			}

			// it's used when user has pressed Esc button to break the loading of the .swf file
			bool get_break_loading() const
			{
				return m_break_loading;
			}
			void break_loading()
			{
				m_break_loading = true;
			}
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		virtual bool is ( int class_id ) const
		{
			...
			{
				return movie_definition_sub::is ( class_id );
			}
		}
		...
		const abc_def *get_abc() const
		{
			return m_abc.get_ptr();
		}
