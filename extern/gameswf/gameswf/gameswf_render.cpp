...
namespace gameswf
{
	...
	namespace render
	{
		...
		bitmap_info	*create_bitmap_info_empty()
		{
			if ( s_render_handler )
			{
				return s_render_handler->create_bitmap_info_empty();
			}
		...
		bitmap_info	*create_bitmap_info_alpha ( int w, int h, unsigned char *data )
		{
			if ( s_render_handler )
			{
				return s_render_handler->create_bitmap_info_alpha ( w, h, data );
			}
		...
		bitmap_info	*create_bitmap_info_rgb ( image::rgb *im )
		{
			if ( s_render_handler )
			{
				return s_render_handler->create_bitmap_info_rgb ( im );
			}
		...
		bitmap_info	*create_bitmap_info_rgba ( image::rgba *im )
		{
			if ( s_render_handler )
			{
				return s_render_handler->create_bitmap_info_rgba ( im );
			}
		...
		video_handler	*create_video_handler()
		{
			if ( s_render_handler )
			{
				return s_render_handler->create_video_handler();
			}

			else
			{
				return NULL;    //hack new bogus_bi;
			}
		}
		...
		void	set_cxform ( const cxform &cx )
		{
			if ( s_render_handler )
			{
				s_render_handler->set_cxform ( cx );
			}
		}
		...
		void draw_triangle_list ( const coord_component coords[], int vertex_count )
		{
			if ( s_render_handler )
			{
				s_render_handler->draw_triangle_list ( coords, vertex_count );
			}
		}
		...
		void	fill_style_bitmap ( int fill_side, bitmap_info *bi, const matrix &m, render_handler::bitmap_wrap_mode wm, render_handler::bitmap_blend_mode bm )
		{
			if ( s_render_handler )
			{
				s_render_handler->fill_style_bitmap ( fill_side, bi, m, wm, bm );
			}
		}
