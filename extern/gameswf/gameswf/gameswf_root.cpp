...
namespace gameswf
{
	...
	movie_definition	*root::get_movie_definition()
	{
	...
	const rgba	&root::get_background_color ( void ) const
	{
	...
	// 0-based!!
	void	root::goto_frame ( int target_frame_number )
	{
		m_movie->goto_frame ( target_frame_number );
	}

	bool	root::has_looped() const
	{
	...
	void	root::set_play_state ( character::play_state s )
	{
		m_movie->set_play_state ( s );
	}
	character::play_state	root::get_play_state() const
	{
	...
	void	root::set_visible ( bool visible )
	{
		m_movie->set_visible ( visible );
	}
	bool	root::get_visible() const
	{
		return m_movie->get_visible();
	}

	void *root::get_userdata()
	{
		return m_userdata;
	}
	void root::set_userdata ( void *ud )
	{
		m_userdata = ud;
	}
