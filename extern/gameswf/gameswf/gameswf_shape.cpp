...
#define SHAPE_LOG 0
		...
		for ( ;; )
		{
			...
			else
			{
				...
				if ( edge_flag == 0 )
				{
					...
					if ( SHAPE_LOG )
					{
						IF_VERBOSE_PARSE ( log_msg ( "  shape_character read: curved edge   = %4g %4g - %4g %4g - %4g %4g\n", x, y, cx, cy, ax, ay ) );
					}
				...
				else
				{
					...
					if ( SHAPE_LOG )
					{
						IF_VERBOSE_PARSE ( log_msg ( "  shape_character_read: straight edge = %4g %4g - %4g %4g\n", x, y, x + dx, y + dy ) );
					}
	    ...
	    const array<line_style>& line_styles )
	{
		...
		for ( int i = 0; i < paths.size(); i++ )
		{
			...
			if ( p.m_fill0 == 0 )
			{
				glColor4f ( 0.25f * p.m_fill1, 0, 0, 0.5 );
			}

			else if ( p.m_fill1 == 0 )
			{
				glColor4f ( 0, 0.25f * p.m_fill0, 0, 0.5 );
			}

			else
			{
				glColor4f ( 0.25f * p.m_fill1, 0.25f * p.m_fill0, 0, 0.5 );
			}
