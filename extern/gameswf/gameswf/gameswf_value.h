...
namespace gameswf
{
	...
	struct as_value
	{
		...
		public:
			...
			exported_module float	to_float() const
			{
				return ( float ) to_number();
			};
			...
			exported_module void	set_int ( int val )
			{
				set_double ( val );
			}
			exported_module void	set_nan()
			{
				set_double ( get_nan() );
			}
			exported_module void	set_as_object ( as_object *obj );
			exported_module void	set_as_c_function ( as_c_function_ptr func );
			exported_module void	set_undefined()
			{
				drop_refs();
				m_type = UNDEFINED;
			}
			exported_module void	set_null()
			{
				set_as_object ( NULL );
			}
			...
			exported_module bool	operator!= ( const as_value &v ) const;
			exported_module bool	operator< ( double v ) const
			{
				return to_number() < v;
			}
			exported_module void	operator+= ( double v )
			{
				set_double ( to_number() + v );
			}
			exported_module void	operator-= ( double v )
			{
				set_double ( to_number() - v );
			...
			exported_module void	operator/= ( double v )
			{
				set_double ( to_number() / v );    // @@ check for div/0
			}
			exported_module void	operator&= ( int v )
			{
				set_int ( int ( to_number() ) & v );
			}
			exported_module void	operator|= ( int v )
			{
				set_int ( int ( to_number() ) | v );
			...
			exported_module void	shl ( int v )
			{
				set_int ( int ( to_number() ) << v );
			}
			exported_module void	asr ( int v )
			{
				set_int ( int ( to_number() ) >> v );
			}
			exported_module void	lsr ( int v )
			{
				set_int ( ( Uint32 ( to_number() ) >> v ) );
			}
			...
			{
				return m_flags & DONT_ENUM ? false : true;
			}
			inline bool is_readonly() const
			{
				return m_flags & READ_ONLY ? true : false;
			}
			inline bool is_protected() const
			{
				return m_flags & DONT_DELETE ? true : false;
			}
			inline int get_flags() const
			{
				return m_flags;
			}
			inline void set_flags ( int flags ) const
			{
				m_flags = flags;
			}
