...
#include <new>	// for placement swfnew
...
public:
	...
	// Set a swfnew or existing value under the key, to the value.
	{
		(*this)[key] = value;
	}
...
#ifdef WIN32
#pragma warning(disable : 4345)	// in MSVC 7.1, warning about placement swfnew POD default initializer
#endif // WIN32
...
public:
	array() : m_buffer(0), m_size(0), m_buffer_size(0), m_stack(false) {}
	array(int size_hint) : m_buffer(0), m_size(0), m_buffer_size(0), m_stack(false) { resize(size_hint); }
	array(T* stack_buffer, int stack_size) :
		m_buffer(stack_buffer),
		m_size(0),
		m_buffer_size(stack_size),
		m_stack(true)
	{
	...
	// Insert the given element at the end of the array.
	{
		...
		assert((T*)&val < &m_buffer[0] || (T*)&val >= &m_buffer[m_buffer_size]);

		const int new_size = m_size + 1;
		if(new_size > m_buffer_size)
		{
			const int new_buffer_size = new_size + (new_size >> 1);
			reserve(new_buffer_size);
		}

		// call constructor with value V into new element.
		new (m_buffer + m_size) T(val);	// placement new
	...
	// of T.  Removed elements are destructed.
	{
		...
		// Copy default T into swfnew elements.
	...
	// realloc?
	{
		...
		if(m_stack)
		{
			// do not allocate/free buffer from the stack
			assert(rsize <= m_buffer_size);
			return;
		}
...
private:
		...
		// of T.  Removed elements are destructed.
	{
		...
		else
		{
			...
			// Copy default T into swfnew elements.
			{for (int i = old_size; i < new_size; i++) {
				swfnew (m_buffer + i) T();	// placement swfnew
			}}
		}
	}
...
public:
	...
	void	add(const T& key, const U& value)
	// Add a swfnew value to the hash table, under the specified key.
	{
		...
		if (natural_entry->is_empty()) {
			// Put the swfnew entry in.
			new (natural_entry) entry(key, value, -1, hash_value);
		} else if (natural_entry->is_tombstone()) {
			// Put the swfnew entry in, without disturbing the rest of the chain.
			...
			if (int(natural_entry->m_hash_value & m_table->m_size_mask) == index)
			{
				...
				new (blank_entry) entry(*natural_entry);	// placement swfnew, copy ctor

				// Put the swfnew info in the natural entry.
			...
			else
			{
				...
				// Put the swfnew data in the natural entry.
				natural_entry->first = key;
				natural_entry->second = value;
				natural_entry->m_hash_value = hash_value;
	...
	// actually contain (which will be less than this).
	{
		...
		if (m_table)
		{
			for (int i = 0, n = m_table->m_size_mask; i <= n; i++)
			{
				...
				if (e->is_empty() == false && e->is_tombstone() == false)
				{
					// Insert old entry into swfnew hash.
...
public:
	tu_string() { 
		...
		//memset(m_local.m_buffer, 0, GAMESWF_STRING_INTERNAL_CAPACITY); 
		invalidate_hash();
		m_permanent = false;
	}
	...
	// If you need a const tu_stringi, don't create a swfnew object;
	...
	inline void update_hashi() const
	{
		m_hashi = bernstein_hash_case_insensitive((const char*)get_buffer(), length());
	}

	inline void invalidate_hash() const
	{
		m_hashi = -1;
	}

	inline int get_hashi() const
	{
		if(m_hashi == -1)
			update_hashi();
...
private:
	...
	// which uses internal storage for strings of 14 (now GAMESWF_STRING_INTERNAL_CAPACITY) characters or
	...
	union
	{
		...
		{
			char	m_size;
			char	m_buffer[GAMESWF_STRING_INTERNAL_CAPACITY];
		} m_local;
...
protected:
	struct
	{
		mutable signed m_hashi : 24;
		unsigned m_permanent : 1;
	};
};
...
public:
	...
	tu_stringi(const tu_stringi& stri) : m_string(stri.to_tu_string()) { }
	...
	int get_hashi() const { return m_string.get_hashi(); }
	bool is_permanent() const { return m_string.is_permanent(); }

private:
	tu_string	m_string;
};

class permanent_string : public tu_string
{
public:
	permanent_string(const tu_string& str) :
		tu_string(str)
	{
		m_permanent = true;
	}

	permanent_string(const char* str) :
		tu_string(str)
	{
		m_permanent = true;
	}
...
public:
	size_t	operator()(const T& data) const
	{
		//const int cache = data.get_hashi();
		//const int hash = bernstein_hash_case_insensitive((const char*)data, data.length());
		//assert(cache == hash);
		return data.get_hashi();
	}
};
...
struct stringi_pointer
{
	stringi_pointer(const tu_stringi* str) :
		m_str(str)
	{
	...
	stringi_pointer(const tu_string* str) :
		m_str(&str->to_tu_stringi())
	{
		//assert(str->is_permanent());
		//m_copy = m_str->c_str();
	}
	...
	inline const tu_string& to_tu_string() const
	{
		//assert(m_copy == *m_str);
		return m_str->to_tu_string();
	}

	inline const tu_stringi& to_tu_stringi() const
	{
		//assert(m_copy == *m_str);
		return *m_str;
	}
	...
	{
		//assert(m_copy == *m_name);
		return m_str->c_str();
	}

	inline int get_hashi() const { return m_str->get_hashi(); }
	inline bool operator==(const stringi_pointer& str) const
	{
		//assert(m_copy == *m_name);
	...
	const tu_stringi* m_str;
	//tu_stringi m_copy;
};

template<class T>
class string_pointer_hash_functor
// Computes a case-insensitive hash of a string-like object (something that has
// ::length() and ::[int] and tolower(::[])).
{
public:
...
// Case-insensitive string hash.
template<class U>
class stringi_pointer_hash : public hash<stringi_pointer, U, string_pointer_hash_functor<stringi_pointer> >
{
};

class permanent_string_cache
{
public:
	~permanent_string_cache()
	{
		clear();
	}

	const tu_string& get(const tu_string& s)
	{
		tu_string* val = NULL;
		if(!m_strings.get(s, &val))
		{
			val = swfnew permanent_string(s);
			m_strings[s] = val;
	...
	void clear()
	{
		for (string_hash<tu_string*>::iterator it = m_strings.begin(); it != m_strings.end(); ++it)
		{
			swfdelete (it->second);
		}
		m_strings.clear();
...
protected:
	string_hash<tu_string*> m_strings;
};
