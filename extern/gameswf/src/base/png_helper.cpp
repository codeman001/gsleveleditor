...
int png_strlen(const char* s)
{
	return strlen(s);
}
int png_memcmp(const void *s1, const void *s2, int numBytes)
{
	//int ret = MEMCMP(s1, s2, numBytes);
	int ret = memcmp(s1, s2, numBytes);
	//char buffer[256];
	//MEMCPY(buffer, s1, numBytes);
	//MEMCPY(buffer + numBytes, s2, numBytes);
	//SPRINTF(buffer + numBytes + numBytes,  " memcmp ret=%d", ret);
	//Log(buffer);
	return ret;
}
void* png_memcpy(void *dstP, const void *sP, int numBytes)
{
	memcpy(dstP, sP, numBytes);
	return dstP;
}
void* png_memset(void* ptr, unsigned char value, int size)
{
