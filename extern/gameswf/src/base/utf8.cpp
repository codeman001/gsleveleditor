...
void	encode_unicode_character(char* buffer, int* index, Uint32 ucs_character)
{
...
void format_utf_text(const char* in, char* out, int auto_line_feed)
{
	while(*in)
	{
		if(*in == ' ')
		{
			...
			case ':':
			case ';':
				encode_unicode_character(out, &index, 0xA0);
				in++;
				out += index;
				continue;
...
bool	check_equal(const char* utf8_in, const Uint32* ucs_in)
{
	for (;;)
	{
		...
		Uint32	next_ucs_from_utf8 = decode_next_unicode_character(&utf8_in);
	...
	if (argc > 1)
	{
		...
		for (;;)
		{
			...
			if (c == '\n' || byte_counter >= LINE_SIZE - 2)
			{
				...
				for (;;)
				{
					Uint32	uc = decode_next_unicode_character(&p);
					*q++ = uc;
				...
				for (;;)
				{
					...
					encode_unicode_character(reencoded_utf8, &index, uc);
