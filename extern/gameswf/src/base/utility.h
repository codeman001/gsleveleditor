...
// On other platform, replace assert by nothing when not in _DEBUG to maximize performance
#ifndef _DEBUG
	#undef assert
	#define assert(x) 
#endif
...
//
// swfnew/delete wackiness -- if USE_DL_MALLOC is defined, we're going to
// try to use Doug Lea's malloc as much as possible by overriding the
// default operator swfnew/delete.
//
#ifdef USE_DL_MALLOC

void*	operator swfnew(size_t size);
...
// Allegedly very good on strings.
{
	...
	while (size > 0) {
		...
		h = ((h << 5) + h) ^ (unsigned) (data[size] >= 'A' && data[size] <= 'Z' ? data[size] + 32 : data[size]);
	}
...
inline double get_nan()
{
	...
	//double zero = 0.0; return zero / zero;
	u.sub.hi = 0xfff80000;
	u.sub.lo = 0;
	u.i = swap_le64(u.i);
	return u.d;
