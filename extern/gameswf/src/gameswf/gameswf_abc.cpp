...
namespace gameswf
{
	...
	void instance_info::read(stream* in, abc_def* abc)
	{
		...
		for (i = 0; i < n; i++)
		{
			traits_info* trait = swfnew traits_info();
			trait->read(in, abc);
			m_trait[i] = trait;
		}
	}
	...
	void	abc_def::read(stream* in, movie_definition_sub* m)
	{
		...
		for (i = 0; i < n; i++)
		{
			as_3_function* info = swfnew as_3_function(this, i, m->get_player());
			info->read(in);
			m_method[i] = info;
		}
		...
		{
			assert(0 && "test");
			metadata_info* info = swfnew metadata_info();
			info->read(in, this);
			m_metadata[i] = info;
		}
		...
		for (i = 0; i < n; i++)
		{
			...
			instance_info* info = swfnew instance_info();
			info->read(in, this);
			m_instance[i] = info;
		}
		...
		for (i = 0; i < n; i++)
		{
			...
			class_info* info = swfnew class_info();
			info->read(in, this);
			m_class[i] = info;
		}
		...
		for (i = 0; i < n; i++)
		{
			...
			script_info* info = swfnew script_info();
			info->read(in, this);
			m_script[i] = info;
		}
