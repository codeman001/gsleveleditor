...
namespace gameswf
{
	...
	// then arg1 is at env->bottom(7), arg2 is at env->bottom(6), etc.
	{
		...
		if (func)
		{
			...
			(*func)(fn_call(&val, this_ptr, env, nargs, first_arg_bottom_index, name));
		}
		else
		{
...
//		log_msg("FIXME(%d): %s\n", __LINE__, __FUNCTION__);
	...
	{
		GAMESWF_PROFILER_SCOPE_EVENT(method_name);
		...
		// Do the call.
		as_value	result = call_method(method, env, this_ptr, nargs, env->get_top_index(), method_name);
		env->drop(nargs);
	...
	struct do_action : public execute_tag
	{
		...
		virtual uint32	get_action_offset() const
		{
			return m_buf.get_stream_position();
		}
	};
	...
	action_buffer::action_buffer() :
		...
		m_buffer(swfnew counted_buffer),
		m_stream_position(0)
	{
	}
	...
	{
		membuf& buf = *m_buffer.get_ptr();
		int action_length = in->get_tag_end_position() - in->get_position();
		int base_offset = buf.size();
		buf.reserve(base_offset + action_length);

		m_stream_position = in->is_compressed() ? in->get_position() + 8 : in->get_position();

		// Read action bytes.
		for (;;)
		{

			int	instruction_start = buf.size();

			int	pc = buf.size();

			int	action_id = in->read_u8();
			buf.append((Uint8) action_id);

			if (action_id & 0x80)
			{
				// Action contains extra data.  Read it.
				int	length = in->read_u16();
				buf.append(Uint8(length & 0x0FF));
				buf.append(Uint8((length >> 8) & 0x0FF));
		...
		//assert(action_length >= buf.size());
	}


	void	action_buffer::process_decl_dict(int start_pc, int stop_pc, player* player)
	...
	// the same action_buffer, then we log an error and ignore it.
	{
		...
		counted_buffer &buffer = *m_buffer.get_ptr();
		...
		if (m_decl_dict_processed_at == start_pc)
		{
			...
			assert(buffer.m_dictionary.size() == count);
			UNUSED(count);
			return;
		}
		...
		buffer.m_dictionary.resize(count);
		...
		for (int ct = 0; ct < count; ct++)
		{
			...
			buffer.m_dictionary[ct] = &player->get_const_string((const char*) &buffer[3 + i]);
			...
			while (buffer[3 + i])
			{
				...
				if (i >= stop_pc)
				{
					...
					while (ct < count)
					{
						buffer.m_dictionary[ct] = &player->get_const_string("<invalid>");
						ct++;
					}
					return;
				}
...
//		env->add_frame_barrier();
		...
		env->get_player()->m_debugger.notify_enter_scope("actions", env, env->m_local_frames.size());
#endif
...
#if GAMESWF_ENABLE_DEBUGGER
		env->get_player()->m_debugger.notify_exit_scope();
#endif
...
//		env->set_local_frame_top(local_stack_top);
	...
	// loads user defined class from DLL / shared library
	{
		...
		{
			lib = swfnew tu_loadlib(classname.c_str());
	...
	// The is_function2 flag determines whether to use global or local registers.
	{
		...
		//printf("action_buffer::execute size=%d \n", abs((int)&address - (int)&start_pc));
		...
		// shared variables
		const as_value undefined;
		const as_value null_value((as_object*)NULL);

		tu_string temp_string;
		...
		for (int pc = start_pc; pc < stop_pc; )
		{
...
#if GAMESWF_ENABLE_DEBUGGER
			env->get_player()->m_debugger.update_script(this, env, pc);
#endif
			...
			if ((action_id & 0x80) == 0)
			{
				...
				case 0x15:	// substring
				{
					...
					// @@ This can be done without swfnew allocations if we get dirtier w/ internals
					// of as_value and tu_string...
					temp_string	= str.c_str() + base;
					temp_string.resize(size);

					env->drop(2);
					env->top(0).set_tu_string(temp_string);
				...
				case 0x17:	// pop
				{
					...
					{
						IF_VERBOSE_ACTION(log_error("error: empty stack\n"));
					}
					break;
				}
				case 0x18:	// int
				...
				{
					temp_string = env->top(1).to_tu_string();
					temp_string += env->top(0).to_tu_string();
					env->drop(1);
					env->top(0).set_tu_string(temp_string);
					break;
				}
				...
				// duplicateMovieClip(target:MovieClip, newname:String, depth:Number) : Void
				{
					...
					if (target == NULL)
					{
						as_value val = env->get_variable(env->top(2).to_string(temp_string), with_stack);
					...
					if (target)
					{
						...
						target->clone_display_object(env->top(1).to_string(temp_string), env->top(0).to_int());
					}
				...
				{
					// Log the stack val.
					as_global_trace(fn_call(&env->top(0), undefined, env, 1, env->get_top_index(), "trace"));
					env->drop(1);
					break;
				}
				...
				case 0x32:	// ord
				{
					...
					env->top(0).set_int(env->top(0).to_string(temp_string).c_str()[0]);
					break;
				}
				case 0x33:	// chr
				{
				...
				case 0x3A:	// delete
				{
					tu_string& varname = temp_string;
					varname = env->top(0).to_tu_string();
					...
					{
						as_value val;
						if (obj_interface->get_member(varname.to_tu_stringi(), &val))
						{
							obj_interface->set_member(varname.to_tu_stringi(), undefined);
							retcode = true;
						}
					}
				...
				case 0x3B:	// delete2
				{
					...
					if (obj.is_undefined() == false)
					{
						...
						else
						{
							...
							as_value val;
							if (env->get_member(varname.to_tu_stringi(), &val))
							{
								env->set_member(varname.to_tu_stringi(), obj);
								env->top(0).set_bool(true);
								break;
							}
						}
				...
				case 0x3D:	// call function
				{
					...
					smart_ptr<character> old_target;
					as_value function;
					as_value this_ptr = null_value;
					if (env->top(0).is_string())
					{
						...
						int with_stack_index = -1;
						function = env->get_variable(function_name, with_stack, &with_stack_index);
						if(with_stack_index != -1)
						{
							old_target = env->get_target();
							character* ch = cast_to<character>(with_stack[with_stack_index].m_object.get_ptr());
							if(ch)
								env->set_target(ch);
						}
						...
						if (function.is_object())
						{
							...
							if (obj)
							{
								...
								if (obj->get_ctor(&function))
								{
									if(function_name == "super")
									{
										this_ptr = obj;
										obj->create_proto(function);
									}
								}
							}
						}
					...
					GAMESWF_PROFILER_BEGIN_EVENT(name);

					int	nargs = env->top(1).to_int();
					as_value	result = call_method(function, env, this_ptr, nargs, env->get_top_index() - 2, name);
					env->drop(nargs + 1);
					env->top(0) = result;

					GAMESWF_PROFILER_END_EVENT();
					
					if(old_target != NULL)
						env->set_target(old_target.get_ptr());
					break;
				}
				case 0x3E:	// return
				{
				...
				case 0x40:	// new
				{
					...
					IF_VERBOSE_ACTION(log_msg("-------------- swfnew object: %s\n",
								  classname.to_tu_string().c_str()));
					int	nargs = env->pop().to_int();
					const tu_string& name = classname.to_string(temp_string);
					as_value constructor = env->get_variable(name, with_stack);
					as_value new_obj;
					...
					if (as_c_function* c_constructor = cast_to<as_c_function>(constructor.to_object()))
					{
						...
						(*c_constructor)(fn_call(&new_obj, null_value, env, nargs, env->get_top_index(), "new"));
					}
					else if (as_s_function* s_constructor = cast_to<as_s_function>(constructor.to_object()))
					{

						// Create an empty object
						smart_ptr<as_object>	new_obj_ptr = swfnew as_object(env->get_player());
						...
						call_method(s_constructor, env, new_obj_ptr.get_ptr(), nargs, env->get_top_index(), name.c_str());
				...
				case 0x42:	// init array
				{
 					...
 					as_value	result;
 					as_global_array_ctor(fn_call(&result, null_value, env, -1, -1, "array"));
 					env->push(result);
					break;
				}
				...
				// property is converted to a string. The value may be of any type.				{
					...
					as_object* obj = swfnew as_object(env->get_player());

					// Set members
					for (int i = 0; i < n; i++)
					{
						obj->set_member(env->top(1).to_tu_stringi(), env->top(0));
						env->drop(2);
					}
				...
				case 0x44:	// type of
				{
					env->top(0).set_string(env->top(0)._typeof());
					break;
				}
				...
				case 0x47:	// add_t (typed)
				{
					...
					{
						tu_string str;
						str = env->top(1).to_string(temp_string);
						str += env->top(0).to_string(temp_string);
				...
				case 0x4B:	// to string
				{
					const tu_string& str = env->top(0).to_string(temp_string);
					env->top(0).set_tu_string(str);
					break;
				}
				case 0x4C:	// dup
				{
					as_value dup(env->top(0));
					env->push(dup);
					break;
				}
				...
				case 0x4E:	// get member
				{
					...
					else
					{
						...
						if(name.is_number())
							...
							last_varname = env->top(0).to_string(temp_string);
#endif

						if(name.is_number())
						{
							obj->get_member(name.to_int(), &(env->top(1)));
						}
						else if (obj->get_member(name.to_string(temp_string).to_tu_stringi(), &(env->top(1))) == false)
						{
							...
							if (obj->get_member("__resolve", &val))
							{
								...
								if (resolve)
								{
									(*resolve)(fn_call(&val, obj, env, 1, env->get_top_index(), "__resolve"));
									env->top(1) = val;
								}
							}
						}
				...
				case 0x4F:	// set member
				{
					...
					if (obj)
					{
						const as_value& name = env->top(1);
						if(name.is_number())
						{
							obj->set_member(name.to_int(), env->top(0));
						}
						else
						{
							obj->set_member(name.to_tu_stringi(), env->top(0));
						}
						IF_VERBOSE_ACTION(
...
//								env->top(2).to_tu_string().c_str(),
				...
				case 0x52:	// call method
				{
					...
					as_value	result;
					const tu_string&	method_name = value.is_string() ? value.to_string(temp_string) : tu_string();
					...
					if (value.is_undefined() || method_name.length() == 0)
					{
						// Flash help says:
						...
						else if (obj && obj->m_this_ptr != NULL)
						{
				...
				// Invokes a constructor function to create a swfnew object.
				// A swfnew object is constructed and passed to the constructor function 
				...
				// The “undefined” return value should be popped off the stack.
					...
					if (as_c_function* c_constructor = cast_to<as_c_function>(constructor.to_object()))
					{
						// C function is responsible for creating the swfnew object and setting members.
						new_obj = call_method( constructor, env, null_value, nargs, env->get_top_index()).to_object();
					}
					else if (as_s_function* s_constructor = cast_to<as_s_function>(constructor.to_object()))
					{
						new_obj = swfnew as_object(env->get_player());
					...
					{
						as_value val;
						if (obj && obj->get_member(constructor.to_tu_stringi(), &val))
						{
...
//								prototype.to_object()->set_member("__constructor__", func);
							...
							else if( as_c_function* c_func = cast_to<as_c_function>(val.to_object()) )
							{
								...
								// Result contains the swfnew output.
								new_obj = call_method(c_func, env, null_value, nargs, env->get_top_index()).to_object();
							}
							else
							{
				...
				case 0x69:	// extends
					...
					// Subclass.prototype = swfnew Object();
					...
					as_object* new_prototype = swfnew as_object(env->get_player());
			...
			else
			{
				...
				case 0x88:	// decl_dict: declare dictionary
				{
					...
					const_cast<action_buffer*>( this )->process_decl_dict(pc, next_pc, env->get_player());
				...
				case 0x8E:	// function2
				{
					...
					tu_string&	name = temp_string;
					name = (const char*) &buffer[i];
					i += name.length() + 1;
					...
					if (name.length() > 0)
					{
						...
						if (env->get_member(name.to_tu_stringi(), &value) && 
					...
					if( !function_exists )
					{
						as_s_function*	func = swfnew as_s_function(env->get_player(), this, next_pc, with_stack);
						...
						if (name.length() > 0)
						{
							...
							env->set_member(name.to_tu_stringi(), function_value);
						}
						else
						{
						...
						switch (type)
						{
							case 0:
							{
								...
								if(len + 1 < 2)
								{
									...
									const tu_string* new_str = &env->get_player()->get_const_string(str);
									env->push(new_str);
								...
								{
									// store index of constant string instead of string itself
									int id = buffer.m_dictionary.size();
									const tu_string* new_str = &env->get_player()->get_const_string((const char*) &buffer[3 + i]);
									//assert(*new_str == str);
									buffer.m_dictionary.push_back(new_str);
									buffer[2 + i] = 0xA;
									buffer[3 + i] = id & 0xFF;
									buffer[4 + i] = id >> 8;
									// pass through
							...
							case 0xA:
							{
								// redirected from case 0 (const char*)
								int id = (buffer[3 + i]) | (buffer[4 + i] << 8);
								const tu_string* str = buffer.m_dictionary[id];
								i += str->length() + 1;
								env->push(str);
								IF_VERBOSE_ACTION(log_msg("-------------- pushed const '%s'\n", str->c_str()));
								break;
							}
							case 1:
							{
	...
	//							}
							...
							case 8:
							{
								int	id = buffer[3 + i];
								i++;
								if (id < buffer.m_dictionary.size())
								{
									// push string pointer

									env->push(buffer.m_dictionary[id]);


									IF_VERBOSE_ACTION(log_msg("-------------- pushed '%s'\n", buffer.m_dictionary[id]->c_str()));
								}
								else
								{
				...
				case 0x9A:	// get url 2
				{
					...
					tu_string buffer;
					const char*	target = env->top(0).to_string(buffer).c_str();
					const char*	url = env->top(1).to_string(temp_string).c_str();
				...
				case 0x9F:	// goto frame expression, goto_frame_exp
				{
					...
					else if (env->top(0).is_string())
					{
						...
						const tu_stringi& frame_label = env->top(0).to_tu_stringi();
	...
	static const char*	s_function_names[event_id::EVENT_COUNT] =
	{
		"INVALID",		 // INVALID
		"onPress",		 // PRESS
		"onRelease",		 // RELEASE
	...
	static array<tu_stringi> s_fnames_string;
	
	void clear_standard_fnames()
	{
		s_fnames_string.clear();
		s_fnames_string.release_buffer();
	}

	const tu_stringi&	event_id::get_function_name() const
	{
		assert(m_id > INVALID && m_id < EVENT_COUNT);

		if(s_fnames_string.size() == 0)
		{
			s_fnames_string.reserve(event_id::EVENT_COUNT);

			for(int i = 0; i < event_id::EVENT_COUNT; i++)
				s_fnames_string.push_back(s_function_names[i]);
		}

		return s_fnames_string[m_id];
	}
