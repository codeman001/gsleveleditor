...
namespace gameswf
{
	...
	void	as_global_trace(const fn_call& fn)
	{
		...
		tu_string buffer;
		log_msg("%s\n", fn.arg(0).to_string(buffer).c_str());

#if GAMESWF_ENABLE_DEBUGGER
		fn.get_player()->m_debugger.trace(fn.arg(0).to_string(buffer).c_str());
#endif
	}
	...
	void  as_global_setinterval(const fn_call& fn)
	{
		...
		{
			object_ref = it.next();
			as_value method_name = it.next();
			
			object_ref.to_object()->get_member(method_name.to_tu_string(), &func);
		}

		if (func.is_function() && it.has_next())
		{
			as_value interval = it.next();
			timer = swfnew as_timer(object_ref, func, interval.to_number(), fn, it.get_pos());
		}
	...
	// Works only for as_object for now
	{
		...
		if (props == NULL)
		{
			...
			for (as_member_hash::const_iterator it = obj->m_members.begin(); 
		...
		else
		{
			...
			for (as_member_hash::const_iterator it = props->m_members.begin(); 
				it != props->m_members.end(); ++it)
			{
				const as_value& key = it->second;
				if (key.is_string())
				{
					as_member_hash::iterator obj_it = obj->m_members.find(key.to_tu_string());
	...
	void	as_global_get_active_controller(const fn_call& fn)
	// Returns an integer with active controller/cursor
	{
		fn.result->set_int(fn.get_root()->get_active_controller());
	}
};
