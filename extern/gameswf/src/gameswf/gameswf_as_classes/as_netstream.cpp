...
namespace gameswf
{
		...
		m_thread(NULL)
	{
		...
		m_thread = swfnew tu_thread(netstream_server, this);
	}
		...
		while (len > 0)
		{
			...
			if (m_sound == NULL)
			{
				...
				if (m_audio_queue.size() > 0)
				{
					...
					m_audio_queue.pop();
					m_sound = swfnew decoded_sound(sample, size);
				}
				else
				{
	...
	void as_netstream::set_status(const char* level, const char* code)
	{
		if (m_is_alive)
		{
			...
			{
				smart_ptr<as_object> infoObject = swfnew as_object(get_player());
...
//				printf("waiting a job...\n");
	...
	bool as_netstream::read_frame()
	{
		...
		while (true)
		{
			...
			{
				m_unqueued_data = swfnew av_packet(packet);
				return read_frame();
			}
		}
	}
...
//		int flg = seek_time < 0 ? AVSEEK_FLAG_BACKWARD : 0;
	...
	{
		as_object* netstream = swfnew as_netstream(fn.get_player());
