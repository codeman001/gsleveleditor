...
namespace gameswf
{
	...
	void string_concat(const fn_call& fn)
	{
		...
		{
			result += fn.arg(i).to_string(buffer);
		}
	...
	void string_split(const fn_call& fn)
	{
		...
		smart_ptr<as_array> arr = swfnew as_array(fn.get_player());
		...
		if (delimiter.size() == 0)
		{
			for (int i = 0; i < max_count; )
			{
				...
				gameswf::decode_next_unicode_character(&n);
				if (n == p)
				{
					break;
				}
		...
		else
		{
			...
			for (int i = 0; i < max_count; )
			{
				bool match = strncmp(p, delimiter.c_str(), delim_size) == 0;
				if (*p == 0 || match)
				{
					// Emit the previous word.
					int size = int(p - word_start);
					working_buffer.resize(imax(working_buffer.size(), size + 1));
					memcpy(&working_buffer[0], word_start, size);
					working_buffer[size] = '\0';
					arr->m_values.push_back(&working_buffer[0]);
					i++;
				...
				{
					decode_next_unicode_character(&p);
				}
			}
		}
	...
	void as_global_string_ctor(const fn_call& fn)
	{
		...
		{
			tu_string buffer;
			fn.result->set_tu_string(fn.arg(0).to_string(buffer));
		}	
		else
		{
