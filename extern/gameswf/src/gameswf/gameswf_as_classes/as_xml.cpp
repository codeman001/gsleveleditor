...
XML::extractNode(xmlNodePtr node, bool mem)
{
  ...
  element = swfnew XMLNode;
            
  //log_msg("Created swfnew element for %s at %p\n", node->name, element);
  ...
  while (attr != NULL) {
    ...
    XMLAttr *attrib = swfnew XMLAttr;
    len = memadjust(strlen(reinterpret_cast<const char *>(attr->name))+1);
    attrib->_name = (char *)swfnew char[len];
    ...
    attrib->_value = (char *)swfnew char[len];
  ...
  element->_name = (char *)swfnew char[len];
    ...
    if (ptr != NULL) {
      if ((strchr((const char *)ptr, '\n') == 0) && (ptr[0] != 0))
      {
        if (node->children->content == NULL) {
          ...
          element->_value = (char *)swfnew char[len];
...
//      log_msg("%sElement at depth %d is %s for node at %p\n", tabs[depth], depth, name, element);
    element->_name = (char *)swfnew char[strlen(reinterpret_cast<const char *>(name))+1];
...
//      log_msg("%sValue at depth %d is \"%s\" for node at %p\n", tabs[depth], depth, value, element);
    element->_value = (char *)swfnew char[strlen(reinterpret_cast<const char *>(value))+1];
  ...
  case XML_READER_TYPE_ATTRIBUTE:
    ...
    element = node;
    XMLAttr *attrib = swfnew XMLAttr;
    attrib->_name = (char *)swfnew char[strlen(reinterpret_cast<const char *>(name))+1];
    memset(attrib->_name, 0, strlen(reinterpret_cast<const char *>(name))+1);
    strcpy(attrib->_name, reinterpret_cast<const char *>(name));
    attrib->_value = (char *)swfnew char[strlen(reinterpret_cast<const char *>(value))+1];
...
//     log_msg("\t\tNo attributes for node %s\n", nodename);
    ...
    attr_obj = swfnew xmlattr_as_object;
...
//                xml->_attributes[i]->_value, nodename, obj);
  ...
  if (length) {
    ...
    for (child=0; child<length; child++) {
      // Create a swfnew AS object for this node's children
      xmlchildnode_obj = swfnew xmlnode_as_object;
