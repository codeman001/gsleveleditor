...
class XMLAttr {
 public:
  ...
  XMLAttr *operator = (XMLAttr node) {
    ...
    _name = swfnew char[strlen(node._name)+2];
    memset(_name, 0, strlen(node._name)+2);
    strcpy(_name, node._name);

    _value = swfnew char[strlen(node._value)+2];
...
class XML {
 public:
  ...
  void cloneNode();                 // Clones the specified node and,
                                    ...
                                    // all children. 
  void createElement(); // Creates a swfnew XML element.
  void createTextNode(); //Creates a swfnew XML text node.
