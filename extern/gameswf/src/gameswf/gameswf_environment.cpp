...
namespace gameswf
{
		...
		switch (get_file_type(fn.c_str()))
		{
...
#else
				image_rgb* im = read_jpeg(fn.c_str());
				if (im)
				{
					bitmap_info* bi = render::create_bitmap_info_rgb(im);
					swfdelete (im);

					bitmap_character*	jpeg = swfnew bitmap_character(get_player(), bi);
			...
			default:
			{
				...
				if(s_texture_loader_callback != NULL)
					texture = s_texture_loader_callback(fn.c_str(), 0, 0);

#ifdef _GLITCH
				if(texture == NULL)
					texture = m_player->m_context->m_video_driver->getTextureManager()->getTexture(fn.c_str()).get();
#endif
				
				if(texture != NULL)
				{
					bitmap_info* bi = get_render_handler()->create_bitmap_info_texture(texture);
					bitmap_character* def = swfnew bitmap_character(get_player(), bi);

					character* ch = m_player->create_generic_character(def, parent, 0);
	...
	as_value	as_environment::get_variable(const tu_string& varname, const array<with_stack_entry>& with_stack, int* with_stack_index) const
	// Return the value of the given var, if it's defined.
	{
		...
		{
			return get_variable_raw(varname, with_stack, with_stack_index);
		}
	}
	...
	// varname must be a plain variable name; no path parsing.
	{
		...
		// Check movie members.
		if (m_target != NULL && m_target->get_member(varnamei, &val))
		{
			return val;
		}

		// Check this, _global, _root
		as_standard_member	varname_id = get_standard_member(varnamei);
		switch (varname_id)
		{
			default:
				break;
			...
			case M_STAGE:
				val.set_as_object(get_player()->get_stage());
				return val;
		...
		if (get_player()->get_global()->get_member(varnamei, &val))
		{
			return val;
		}
	...
	// No path rigamarole.
	{
		...
		for (int i = with_stack.size() - 1; i >= 0; i--)
		{
			as_object*	obj = with_stack[i].m_object.get_ptr();
			as_value unused;
			if (obj && obj->get_member(varnamei, &unused))
			{
				// This object has the member; so set it here.
				obj->set_member(varnamei, val);
				return;
			}
		}
		...
		if (m_target != NULL)
		{
			m_target->set_member(varnamei, val);
		}
		else
		{
			// assume local var
	...
	// Set/initialize the value of the local variable.
	{
		...
		if (index < 0)
		{
			// Not in frame; create a swfnew local var.
			add_local(varname, val);
		}
		else
		{
	...
	as_object*	as_environment::find_target(const char* target) const
	{
		if (m_target != NULL)
		{
			return m_target->find_target(target);
