...
namespace gameswf
{
		...
		if (fn.nargs > 0)
		{
			...
			if (properties->m_this_ptr != NULL)
			{
				...
				if (func)
				{
					...
					// push 1..n arguments, arg 0 is "thisObject"
					int nargs = fn.nargs - 1;
		...
		m_function2_flags(0)
	{
		...
		m_prototype = swfnew as_object(player);
		//builtin_member("call", player->m_as_s_function_call);
		// NOW overiding bool get_member(const tu_stringi& name, as_value* val);
	}
	...
	// Dispatch.
	{
		...
		else
		{
			...
			{
				arg_array = swfnew as_array(env->get_player());

				for (int i = 0; i < fn.nargs; i++)
				{
					arg_array->set_member(i, fn.arg(i));
				}
			}
...
//		printf("***on entry*** %d\n", stack_size);
		...
		fn.env->get_player()->m_debugger.notify_enter_scope(fn.name, env, local_stack_top);
#endif
...
#if GAMESWF_ENABLE_DEBUGGER
		fn.env->get_player()->m_debugger.notify_exit_scope();
#endif
