...
//				printf("signal for frame %d\n", m_loading_frame);
		...
		virtual bool	get_labeled_frame(const tu_stringi& label, int* frame_number) = 0;
		...
		virtual bool	has_init_actions() = 0;
		...
		// loading
		hash<int, cxform*> m_cxform_cache;
		hash<int, matrix*> m_matrix_cache;
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		stringi_pointer_hash<int>	   m_named_frames;	// 0-based frame #'s
		...
		bool	get_labeled_frame(const tu_stringi& label, int* frame_number);
		...
		bool	has_init_actions() { return m_has_init_actions; }
