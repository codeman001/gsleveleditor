...
#define ENABLE_OPTIMIZED_WORLD_MATRIX_CXFORM 1
...
namespace gameswf
{
	...
	typedef stringi_pointer_hash<as_value> as_member_hash;
	//typedef stringi_hash<as_value> as_member_hash;
	...
	struct as_object : public as_object_interface
	{
		...
		// it's used for passing swfnew created object pointer to constructors chain
		...
		exported_module virtual const char*	_typeof() { return "object"; }
		...
		exported_module as_object* find_target(const char* target);
		...
		exported_module virtual void notify_set_member(const tu_stringi& name, const as_value& new_val) { }
