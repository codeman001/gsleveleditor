...
//		memcpy(&m_triangle_strip[0], &pts[0], count * sizeof(point));
	...
	void mesh::set_triangles(const coord_component* pts, int pts_count, const unsigned short* indexes, int index_count)
	{
		m_triangle_list.append(pts, pts_count);
		m_index_list.append(indexes, index_count);
	}
	...
	void	mesh::display(const base_fill_style& style, float ratio) const
	{
		...
		if (m_triangle_list.size() > 0) {
			style.apply(0, ratio);
			render::draw_triangle_list(&m_triangle_list[0], m_triangle_list.size() >> 1, &m_index_list[0], m_index_list.size());
		}
	}
...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	struct tri_stripper
	{
		...
		// Add two triangles to our strip.
		{
			...
			// it's almost certainly the start of a swfnew
			// strip.  Don't bother searching current
			// strips.

			// Can't join with existing strip, so start a swfnew strip.
		...
		// Add a triangle to our strip.
		{
			...
			// produced by the swfnew triangulator.
			//
			// Dumb temp code!
			if (m_strips.size() == 0) {
				// Start a swfnew strip.
				m_strips.resize(m_strips.size() + 1);
			}
		...
		m_error_tolerance(error_tolerance)
	{
...
#if GAMESWF_ENABLE_LIBTESS
		shape_character_def* scd = (shape_character_def*)sh;
		execute_tesselate(this, scd, error_tolerance);
#else
		...
		// For collecting triangles emitted by the swfnew tesselator.
	...
	mesh_set::layer::~layer() {
		for (int i = 0; i < m_line_strips.size(); i++) {
			swfdelete (m_line_strips[i]);
		}
		for (int i = 0; i < m_meshes.size(); i++) {
			swfdelete (m_meshes[i]);
		}
	}


	void mesh_set::new_layer()
	// Make room for a swfnew layer.
	{
	...
	// Add the specified line strip to our list of things to render.
	{
		...
		m_layers.back().m_line_strips.push_back(swfnew line_strip(style, coords, coord_count));
	}
	...
	// Grab our data from the input stream.
	{
		...
		for (int j = 0; j < layer_n; j++) {
			...
			{for (int i = 0; i < lines_n; i++)
			{
				l->m_line_strips[i] = swfnew line_strip;
	...
	// Read fill styles, and push them onto the given style array.
	{
		...
		{
			int base_count = styles->size();
			styles->resize(base_count + fill_style_count);

			for (int i = 0; i < fill_style_count; i++)
			{
				(*styles)[base_count + i].read(in, tag_type, m);
			}
		}
	}
	...
	// Read line styles and push them onto the back of the given array.
	{
		...
		{
			int base_count = styles->size();
			styles->resize(base_count + line_style_count);
	...
	shape_character_def::~shape_character_def()
	{
		...
		{
			swfdelete (m_cached_meshes[i]);
		}
	}


	character*	shape_character_def::create_character_instance(character* parent, int id)
	{
		return get_player()->create_generic_character(this, parent, id);
	}
...
#define SHAPE_LOG 0
		...
		for (;;) {
			...
			if (type_flag == 0)
			{
				...
				if (flags & 0x10) {
					...
					// Tack on an empty path signalling a swfnew shape.
					// @@ need better understanding of whether this is correct??!?!!
					// @@ i.e., we should just start a whole swfnew shape here, right?
	...
	{
		const matrix&	mat = inst->get_world_matrix();
...
// 		}
		...
		// Construct a swfnew mesh to handle this error tolerance.
		mesh_set*	m = swfnew mesh_set(this, object_space_max_error * 0.75f);
