...
namespace gameswf
{
	...
	// Load a DefineSound tag.
	{
		...
		if (s_sound_handler)
		{
			...
			if (format == sound_handler::FORMAT_ADPCM)
			{
				...
				data = swfnew unsigned char[data_bytes];
			...
			int	handler_id = s_sound_handler->create_sound(
				...
				stereo);
			sound_sample*	sam = swfnew sound_sample(m->get_player(), handler_id);
			m->add_sound_sample(character_id, sam);

			swfdelete_primitives (data, data_bytes);
		}
	}
	...
	// Load a StartSound tag.
	{
		...
		if (sam)
		{
			start_sound_tag*	sst = swfnew start_sound_tag();
...
#define DO_SAMPLE(n_bits, sample, stepsize_index, raw_code)									\
	{															\
		...
		int	stepsize = s_stepsize[stepsize_index];									\
																\
		/* Compute the swfnew sample.  It's the predicted value			*/					\
