...
namespace gameswf
{
	...
	sprite_instance::sprite_instance(player* player, movie_definition_sub* def,	root* r, 
		...
		character(player, parent, id, AS_SPRITE),
		m_def(def),
		m_root(r),
		m_play_state(PLAY),
		m_current_frame(0),
		m_update_frame(true),
		m_has_enter_frame(false),
		m_init_actions(NULL),
		m_as_environment(NULL),
		m_mouse_state(UP),
		m_enabled(true),
		...
		m_script(NULL)
	{
		...
		if (m_def->has_init_actions())
		{
			m_init_actions = swfnew init_actions();
			m_init_actions->m_executed.resize(m_def->get_frame_count());
			memset(&(m_init_actions->m_executed[0]), 0,
				sizeof(m_init_actions->m_executed[0]) * m_init_actions->m_executed.size());
		}

		m_player->set_alive(this);
		set_ctor(m_player->m_as_global_movieclip_ctor);
	}
...
//		printf("delete sprite 0x%p\n", this);
		...
		if(m_init_actions != NULL)
			swfdelete (m_init_actions);

		if(m_as_environment != NULL)
			swfdelete (m_as_environment);
	}

	void sprite_instance::recycle(character* parent, int id)
	{
	...
	void sprite_instance::get_bound(rect* bound)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			if (ch != NULL)
			{
				...
				if(ch_bound.width() > 0 && ch_bound.height() > 0)
				{
	...
	character* sprite_instance::add_empty_movieclip(const char* name, int depth)
	{
		...
		sprite_definition* empty_sprite_def = swfnew sprite_definition(get_player(), NULL);

		sprite_instance* sprite =	get_player()->create_sprite_instance(empty_sprite_def, m_root, this, 0);
	...
	{
		return (sprite_instance::play_state)m_play_state; 
	}
	...
	// Return true if we have any mouse event handlers.
	{
		if (is_enabled())
		{
			if(m_can_handle_mouse_event)
				return true;
			else if(sprite_can_handle_mouse_event_callback(this))
				return true;
		}
	...
	// none.  Coords are in parent's frame.
	{
...
#ifdef _GLITCH
		if(m_custom != NULL && m_custom->m_scene_node != NULL)
		{
			m_custom->m_scene_node->get_local_mouse(this, x, y);
		}
#endif
		...
		for (i = n - 1; i >= 0; i--)
		{
			...
			if (ch != NULL && ch->get_visible())
			{
				...
				if (strcmp(ch->m_name.c_str(), "hitzone") == 0)
				{
					break;
				}
			}
	...
	void sprite_instance::construct()
	{
		if (m_constructor_called == false)
		{
			m_def->instanciate_registered_class(this);
			m_display_list.construct();
	...
	//
	// 2. Create a .swf file that isolates the bug or swfnew feature.
	...
	// Macromedia flash player.  Create a swfnew test file under
	// tests/ where the first line is the name of the swfnew test
	// .swf, and the rest of the file is the correct trace()
	// output.
	//
	// 4. Verify that gameswf fails the swfnew test (by running
	// ./gameswf_batch_test.py tests/name_of_new_test.txt)
	//
	// 5. Fix gameswf behavior.  Add the swfnew test to
	...
	// OK!  If not, then the swfnew behavior is actually a
	// regression.)
	void sprite_instance::advance(float delta_time)
	{
		...
		{
			// ensure instance and child are constructed
			{
				GAMESWF_PROFILER_SCOPE_EVENT("construct");
				construct();
			}
		...
		if(!m_visible && m_on_event_load_called)
		{
			return;
		}

		m_need_advance = false;
		m_need_advance |= m_goto_frame_action_list.size() > 0;
		...
		int recurseDepth = 0;
		while (m_goto_frame_action_list.size() > 0)
		{
			// try to use stack buffer if less than 32 objects, use heap buffer otherwize
			action_buffer* stack_buffer[32] = {0};
			array<action_buffer*> stack_actions(stack_buffer, 32);
			array<action_buffer*> heap_actions;
			array<action_buffer*>& temp_goto_frame_action_list = m_goto_frame_action_list.size() < 32 ? stack_actions : heap_actions;

			temp_goto_frame_action_list = m_goto_frame_action_list;
			m_goto_frame_action_list.clear();
			execute_actions(get_environment(), temp_goto_frame_action_list);

			if(recurseDepth++ > 10)
			{
				gameswf::log_msg("stop recursive loop gotoAndPlay/gotoAndStop (more than 10)");
				break;
			}
		}
		...
		{
			m_need_advance |= m_def->get_frame_count() > 1;
			...
			{ 
				...
				if (m_current_frame == 0 && m_def->get_frame_count() > 1)
				{
					...
					// try to use stack buffer if less than 32 objects, use heap buffer otherwize
					int stack_buffer[32] = {0};
					array<int> stack_affected_depths(stack_buffer, 32);
					array<int> heap_affected_depths;
					array<int>& affected_depths = playlist.size() < 32 ? stack_affected_depths : heap_affected_depths;
		...
		if(m_display_list.advance(delta_time))
			m_need_advance = true;
	...
	void sprite_instance::notify_set_member(const tu_stringi& name, const as_value& new_val)
	{
		// monitor the onEnterFrame member
		if(strcmp(name.c_str(), "onEnterFrame") == 0)
		{
			m_has_enter_frame = true;
			notify_need_advance();
		}
		else if(strncmp(name.c_str(), "on", 2) == 0)
		{
	...
	// frame is 0-based
	{
		...
		if (m_init_actions != NULL && m_init_actions->m_executed[frame] == false)
		{
			...
			if (init_actions && init_actions->size() > 0)
			{
				...
				// again.
				m_init_actions->m_executed[frame] = true;
			}
			// transfert init actions
			m_init_actions->m_action_list.append(m_action_list);
			m_action_list.clear();
		}
	...
	// Take care of this frame's actions.
	{
		if(m_action_list.size() > 0)
		{
			...
			// Put actions into temporary list and clear m_action_list immediately
			// because action can call gotoAndPlay / gotoAndStop which will modify m_action_list.

			// try to use stack buffer if less than 32 objects, use heap buffer otherwize
			action_buffer* stack_buffer[32] = {0};
			array<action_buffer*> stack_actions(stack_buffer, 32);
			array<action_buffer*> heap_actions;
			array<action_buffer*>& temp_action_list = m_action_list.size() < 32 ? stack_actions : heap_actions;
			
			temp_action_list = m_action_list;
			m_action_list.resize(0);
			execute_actions(get_environment(), temp_action_list);
		}
		...
		if (m_frame_script != NULL)
		{
			...
			gameswf::call_method(m_frame_script.get_ptr(), get_environment(), this, 0, 0);
			m_frame_script = NULL;
		}
	}

	void	sprite_instance::do_init_actions()
	{
		if(m_init_actions != NULL)
		{
			// Keep m_as_environment alive during any method calls!
			smart_ptr<as_object>	this_ptr(this);
			execute_actions(get_environment(), m_init_actions->m_action_list);
			m_init_actions->m_action_list.clear();
		}
	}
	...
	void sprite_instance::do_actions(const array<action_buffer*>& action_list)
	{
		...
		{
			action_list[i]->execute(get_environment());
		}
	}
	...
	bool	sprite_instance::goto_frame(const tu_string& target_frame)
	{
		...
		{
			return goto_labeled_frame(target_frame.to_tu_stringi());
		}
	...
	// 0-based frame numbers!!  (in contrast to ActionScript and Flash MX)
	{
		...
		// Appends actions of target frame so they are executed before normal actions
		m_goto_frame_action_list.append(m_action_list);
		m_action_list.clear();
		notify_need_advance();
		return true;
	}
	...
	int		sprite_instance::get_frameid_from_label(const tu_stringi& label)
	{
		int	target_frame = -1;
	...
	bool	sprite_instance::goto_labeled_frame(const tu_stringi& label)
	...
	void	sprite_instance::display()
	{
		...
		if(m_display_list.size() != 0)
		{
			blend_mode::id mode	= get_blend_mode();
			bool set = mode != blend_mode::NORMAL && m_player->m_blend_stack.size() == 0;
			
			if(set)
			{
				m_player->m_blend_stack.push_back(mode);
				render::set_blend_mode(mode);
			}
			...
			if(set)
			{
				m_player->m_blend_stack.pop_back();
				render::set_blend_mode(blend_mode::NORMAL);
			}
		}
		...
		// Add an object to the display list.
	{
		...
		{
			move_display_object(depth, color_transform, matrix, eff, ratio, clip_depth);
			return NULL;
		}
	...
	void	sprite_instance::move_display_object(
		...
		// the specified depth.
	{
		m_display_list.move_display_object(depth, color_xform, mat, eff, ratio, clip_depth);
	}
...
//			ch->on_event(event_id::LOAD);
		...
		sprite_instance* sprite = get_player()->create_sprite_instance(cast_to<movie_def_impl>(md), 
			get_root(),	parent,	-1);
	...
	void	sprite_instance::set_variable(const char* path_to_var, const char* new_value)
	{
		...
		get_environment()->set_variable(path, val, empty_with_stack);
	}
	...
	const char*	sprite_instance::get_variable(const char* path_to_var) const
	{
		...
		val = ((sprite_instance*)this)->get_environment()->get_variable(path, empty_with_stack);
	...
	bool	sprite_instance::set_member(const tu_stringi& name, const as_value& val)
	{
		...
		switch (std_member)
		{
			...
			case M_ENABLED:
			{
				...
				{
					gameswf::call_method(method, get_environment(), this, 0, 
			...
			case M_TRANSFORM:
			{
				as_transform* trans = cast_to<as_transform>(val.to_object());
				if(trans != NULL)
				{
					trans->copy_to(get_transform());
					get_transform()->m_movie = this;
					
					as_value val;
					trans->get_member("matrix", &val);

					as_matrix* mat = cast_to<as_matrix>(val.to_object());
					if(mat != NULL)
						set_matrix(mat->m_matrix);

					val.set_undefined();
					trans->get_member("colorTransform", &val);

					as_color_transform* trans = cast_to<as_color_transform>(val.to_object());
					if(trans != NULL)
						set_cxform(trans->m_color_transform);
				}
				return true;
			}
		}
	...
	{
		//printf("sprite_instance::get_member %s\n", name.c_str());
		...
		// try standart character properties & movieclip variables
		...
		switch (std_member)
		{
			...
			{
				get_transform()->init();
				val->set_as_object(get_transform());
				return true;
			}
			default:
				break;
	...
	// frame_spec could be an integer or a string.
	{
		...
		{
			if (m_def->get_labeled_frame(frame_spec.to_tu_stringi(), &frame_number) == false)
			{
		...
		// Execute any swfnew actions triggered by the tag,
		// leaving existing actions to be executed.
		while (m_action_list.size() > top_action)
		{
			m_action_list[top_action]->execute(get_environment());
	...
	void	sprite_instance::set_drag_state(const drag_state& st)
	{
		...
		if(st.m_character)
			st.m_character->notify_need_advance();
	}
	...
	// Duplicate the object with the specified name and add it with a swfnew name 
	// at a swfnew depth.
	{
		...
		{ 
			...
			if (get_id() == -1)	
			{
				ch = get_player()->create_sprite_instance(cast_to<movie_def_impl>(m_def.get_ptr()), 
					get_root(),	parent,	-1);
			...
			{
				ch = get_player()->create_sprite_instance(m_def.get_ptr(), get_root(),	parent,	0);
				ch->set_parent(parent);
				ch->set_root(get_root());
				ch->set_name(newname);
			...
			ch->set_cxform(get_cxform());
			ch->set_matrix(get_matrix());
			ch->set_effect(get_effect());

			// copy this's members to swfnew created character
			copy_to(ch);
	...
	// Dispatch event handler(s), if any.
	{
		...
		as_value	method;
		if (as_object::get_member(method_name, &method))
		{
			int nargs = 0;
			if (id.m_args)
			{
				...
				{
					get_environment()->push((*id.m_args)[i]);
				}
			}

			GAMESWF_PROFILER_SCOPE_EVENT(method_name.c_str());

			gameswf::call_method(method, get_environment(), this, nargs, 
				get_environment()->get_top_index(), method_name.c_str());

			get_environment()->drop(nargs);
			return true;
		}
		return false;
	}
	...
	const char*	sprite_instance::call_method_args(const char* method_name, const char* method_arg_fmt, va_list args)
	{
		...
		return call_method_parsed(get_environment(), this, method_name, method_arg_fmt, args);
	}
	
	tu_string	sprite_instance::call_method(const char* method_name, as_value * arguments, int argument_count )
	{
		return gameswf::call_method( get_environment(), this, method_name, arguments, argument_count );
	}

	void	sprite_instance::attach_display_callback(const char* path_to_object, void (*callback)(render_state&, void*), void* user_ptr)
	{
		assert(m_parent == NULL);	// should only be called on the root movie.

		array<with_stack_entry>	dummy;
		as_value	obj = get_environment()->get_variable(tu_string(path_to_object), dummy);
	...
	bool	sprite_instance::hit_test(character* ch)
	{
		...
		get_bound(&r);
		if(get_parent() != NULL)
			get_parent()->get_world_matrix().transform(&r);

		rect ch_r;
		ch->get_bound(&ch_r);
		if(ch->get_parent() != NULL)
			ch->get_parent()->get_world_matrix().transform(&ch_r);

		if((r.m_y_max < ch_r.m_y_min) ||
			(r.m_y_min > ch_r.m_y_max) ||
			(r.m_x_max < ch_r.m_x_min) ||
			(r.m_x_min > ch_r.m_x_max))
			return false;

		return true;
	}

	bool	sprite_instance::hit_test(float x, float y, bool shape)
	{
		if(shape)
		{
			// consider shape as hit area
			if (get_visible() == false)
				return false;
		...
		{
			// consider bounds as hit area
			rect bound;
			get_bound(&bound);
			if(get_parent() != NULL)
				get_parent()->get_world_matrix().transform(&bound);
			return bound.point_test(x, y);
		}
	}

	bool	sprite_instance::hit_test(float x, float y)
	{
		return hit_test(x, y, true);
	}
	...
	// Creates a swfnew, empty text field as a child of the movie clip
	{
		edit_text_character_def* textdef = swfnew edit_text_character_def(get_player(), width, height);
		...
		m_display_list.add_display_object(textfield, depth, true, &cxform::identity, &matrix::identity, &effect::identity, 0.0f, 0); 
		textfield->set_matrix(m);
...
//		visited_objects->set(this, true);
	...
	as_environment*	sprite_instance::get_environment()
	{
		// ensure environment is allocated
		if(m_as_environment == NULL)
		{
			m_as_environment = swfnew as_environment(get_player());
		...
		int depth)
	{
		...
		sprite_instance* sprite = get_player()->create_sprite_instance(sdef, get_root(), this, -1);
		sprite->set_name(name);
	...
	canvas* sprite_instance::get_canvas()
	{
		if (m_canvas == NULL)
		{
			canvas* canvas_def = swfnew canvas(get_player());
	...
	as_transform* sprite_instance::get_transform()
	{
		if(m_transform == NULL)
			m_transform = swfnew as_transform(get_player(), this);
		
		return m_transform.get_ptr();
	}
	...
	// frame is 0-based
	{
		if (frame >= 0 && frame < m_def->get_frame_count())
		{
			if (m_script == NULL)
			{
				m_script = swfnew hash<int, smart_ptr<as_function> >;
			}
	...
	void sprite_instance::update_world_matrix()
	{
		character::update_world_matrix();
	...
	void sprite_instance::update_world_cxform()
	{
