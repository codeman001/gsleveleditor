...
namespace gameswf
{
	...
	struct sprite_definition : public movie_definition_sub
	{
		...
		stringi_pointer_hash<int>	 m_named_frames;	// stores 0-based frame #'s
...
//		virtual void	generate_font_bitmaps() { assert(0); }
		...
		{
			stringi_pointer n(label);
			return m_named_frames.get(n, frame_number);
		}
