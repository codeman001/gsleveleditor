...
//			int gradient_count = in->read_uint(4);
	...
	// We can use this to set the gradient fill style.
	{
		...
		if (m_type == 0x10)
		{
			// Linear gradient.
#ifdef IPHONEOS
			// Note: iPhone seems to have problem using 256x1 texture, so we create 256x8 instead
			#define LINEAR_GRADIENT_HEIGHT 8
#else
			#define LINEAR_GRADIENT_HEIGHT 1
#endif
			im = create_rgba(256, LINEAR_GRADIENT_HEIGHT);

			for (int i = 0; i < im->m_width; i++)
			{
				rgba	sample = sample_gradient(i);

				for(int j = 0; j < LINEAR_GRADIENT_HEIGHT; ++j)
				{
	...
	// Push our style parameters into the renderer.
	{
		...
		else if (m_type >= 0x40 || m_type <= 0x43)
		{
			...
			if (m_bitmap_character != NULL)
			{
				...
				if (bi != NULL)
				{
					gameswf::render_handler::bitmap_wrap_mode	wmode = gameswf::render_handler::WRAP_CLAMP;
					if (m_type == 0x42)
					{
						wmode = gameswf::render_handler::WRAP_REPEAT;
					}
					gameswf::render::fill_style_bitmap(
						fill_side,
						bi,
