...
// index, and outputs trapezoids (old code) or triangles (swfnew code).
//
...
// have holes though.
//
// Current outlook: swfnew triangulator looks good so far.  Need to clean
// up & optimize the interface a bit, and look for pathological cases
// where it does something wrong.  The swfnew triangulator needs more
...
namespace tesselate
{
	...
	// Draw our shapes and lines, then clear the segment list.
	{
		...
		s_current_segments.release_buffer();
	}
	...
	// trapezoids.  Modifies the values in s_current_segments.
	{
		...
		// Add by FengCheng [3/22/2009]
		for (int i = 0; i < slab.size() - 1; i++)
		{
			if (slab[i].m_end.m_x > slab[i + 1].m_end.m_x)
			{
				if (fabsf(slab[i].m_begin.m_x - slab[i + 1].m_begin.m_x) < fabsf(slab[i].m_end.m_x - slab[i + 1].m_end.m_x))
				{
					// in case:
					//	  \|
					//	   *
					...
					//	   |  \
					//
					swap(&slab[i].m_begin.m_x, &slab[i + 1].m_begin.m_x);
					swap(&slab[i], &slab[i + 1]);
				}
				else
				{
					// in case:
					...
					//	   |\
					//
					swap(&slab[i].m_end.m_x, &slab[i + 1].m_end.m_x);
				}
			}
			else if (slab[i].m_begin.m_x == slab[i + 1].m_begin.m_x && slab[i].m_end.m_x == slab[i + 1].m_end.m_x &&
				slab[i].m_left_style == slab[i + 1].m_right_style && slab[i].m_right_style == slab[i + 1].m_left_style)
			{
				// in case:
				//	    ||
				//	    ||
	...
	// the given swfnew anchor point (ax, ay), with (cx, cy) acting
...
namespace tesselate_new
{
	...
	void	end_shape()
	{
		...
		// Convert left-fill paths into swfnew right-fill paths,
...
// http://www.emi.u-bordeaux.fr/imm/glu/tesscallback.html
...
struct tesselator_accepter_allocator
{
	const static int MAX_SIZE = 4096 * 2;
	vertex3d m_buffer[MAX_SIZE];
	int m_size;

	tesselator_accepter_allocator() :
		m_size(0)
	{
	}
	...
	vertex3d* allocate(GLdouble x = 0, GLdouble y = 0, GLdouble z = 0)
	{
		assert(m_size < MAX_SIZE);

		m_buffer[m_size].coords[0] = x;
		m_buffer[m_size].coords[1] = y;
		m_buffer[m_size].coords[2] = z;
		return &m_buffer[m_size++];
	}
};

static tesselator_accepter_allocator s_tesselator_accepter_allocator;

struct tesselator_accepter
{
	GLenum m_type;
	array<point> m_temp_points;
	
	array<point> m_final_points;
	array<unsigned short> m_final_indexes;

	bool m_contour;
	bool m_use_strips;
	GLUtesselator* m_tess;

	tesselator_accepter(bool use_strips, bool contour, bool intersection) :
		m_contour(contour),
		m_use_strips(use_strips)
	{
		m_tess = gluNewTess();
		gluTessProperty(m_tess, GLU_TESS_WINDING_RULE, intersection ? GLU_TESS_WINDING_ABS_GEQ_TWO : GLU_TESS_WINDING_ODD);
		gluTessProperty(m_tess, GLU_TESS_BOUNDARY_ONLY, contour ? 1 : 0);
		gluTessCallback(m_tess, GLU_TESS_BEGIN_DATA,	(void (GLAPI *)())tesselator_accepter::callback_begin);
		gluTessCallback(m_tess, GLU_TESS_VERTEX_DATA,	(void (GLAPI *)())tesselator_accepter::callback_vertex);
		gluTessCallback(m_tess, GLU_TESS_COMBINE_DATA,	(void (GLAPI *)())tesselator_accepter::callback_combine);
		gluTessCallback(m_tess, GLU_TESS_ERROR_DATA,	(void (GLAPI *)())tesselator_accepter::callback_error);
		gluTessCallback(m_tess, GLU_TESS_END_DATA,		(void (GLAPI *)())(use_strips ? tesselator_accepter::callback_end_strip : tesselator_accepter::callback_end_tris));
	}
	...
	void init()
	{
		s_tesselator_accepter_allocator.init();
		
		m_temp_points.clear();
		m_final_points.clear();
		m_final_indexes.clear();
	}
	...
	void begin_shape()
	{
		...
		if(m_contour)
			tesselator_accepter::callback_begin(GL_LINE_STRIP, this);
	}
	...
	void end_shape(mesh_set* m, int style)
	{
		gluTessEndPolygon(m_tess);

		if(m_contour)
			tesselator_accepter::callback_end_tris(this);

		if(m_contour)
		{
			// add line
			if(m_final_points.size() > 1)
				m->add_line_strip(style, &(m_final_points[0]), m_final_points.size());
		}
		else
		{
			if(m_use_strips)
			{
				// add strip
				if(m_final_points.size() > 0)
					m->set_tri_strip(style, &(m_final_points[0]), m_final_points.size());
			}
			else
			{
				// add indexed triangles
				if(m_final_points.size() > 0)
					m->get_mutable_mesh(style)->set_triangles(static_cast<coord_component*>(&m_final_points[0].m_x), m_final_points.size() * 2, &m_final_indexes[0], m_final_indexes.size());
			}
		}
		m_final_points.clear();
	}

	void end_shape(array<point>& result, array<unsigned short>& indexes)
	{
		gluTessEndPolygon(m_tess);
		...
		{
			// add strip
			result = m_final_points;
		}
		else
		{
			// add indexed triangles
			result = m_final_points;
			indexes = m_final_indexes;
		}
	...
	{
		vertex3d* v = s_tesselator_accepter_allocator.allocate(p.m_x, p.m_y);

		if(m_contour)
			callback_vertex(v, this);
		else
			gluTessVertex(m_tess, v->coords, v->coords);
	}
	...
	static void callback_begin(GLenum _type, tesselator_accepter* ta)
	{
		ta->m_type = _type;
		ta->m_temp_points.clear();
	}

	static void callback_vertex(const void* vertex_data, tesselator_accepter* ta)
	{
		vertex3d* v = (vertex3d*)vertex_data;
		point p((float)v->coords[0], (float)v->coords[1]);
		
		//printf("v=0x%x (%.1f, %.1f)\n", v, p.m_x, p.m_y);
		ta->m_temp_points.push_back(p);
	}

	static void callback_combine(GLdouble coords[3], void* vertex_data[4], GLfloat weight[4], void** outData, tesselator_accepter* ta)
	{
		vertex3d* v = s_tesselator_accepter_allocator.allocate();

		v->coords[0] = coords[0];
		v->coords[1] = coords[1];
		v->coords[2] = coords[2];
		//v->r =	w[0]*d[0]->r + w[1]*d[1]->r + w[2]*d[2]->r + w[3]*d[3]->r;
		//v->g =	w[0]*d[0]->g + w[1]*d[1]->g + w[2]*d[2]->g + w[3]*d[3]->g;
		//v->b =	w[0]*d[0]->b + w[1]*d[1]->b + w[2]*d[2]->b + w[3]*d[3]->b;
		//v->a =	w[0]*d[0]->a + w[1]*d[1]->a + w[2]*d[2]->a + w[3]*d[3]->a;
		*outData	= v;

		//printf("o=0x%x (%.1f, %.1f)\n", v->coords[0]/20, v->coords[1]/20);
	}

	static void callback_error(GLenum _errno, tesselator_accepter* ta)
	{

	}

	static void callback_end_strip(tesselator_accepter* ta)
	{
		if(ta->m_temp_points.size() == 0)
			return;

		if(ta->m_final_points.size() > 0)
		{
			// degenerate #1
			point deg = ta->m_final_points.back();
			ta->m_final_points.push_back(deg);
		}

		//printf("callback_end\n");
		//for(int i = 0; i < ta->m_temp_points.size(); i++)
		//	printf("point (%.1f, %.1f)\n", ta->m_temp_points[i].m_x/20, ta->m_temp_points[i].m_y/20);
		...
		case GL_TRIANGLES:
			...
			if(ta->m_final_points.size() > 0)
			{
				ta->m_final_points.push_back(ta->m_temp_points[0]);
			}
			// convert triangles to triangle strip
			for(int i = 0; i < ta->m_temp_points.size(); i+=3)
			{
				if(i > 0)
				{
					point deg = ta->m_final_points.back();
					ta->m_final_points.push_back(deg);
					ta->m_final_points.push_back(ta->m_temp_points[i+0]);
				}

				ta->m_final_points.push_back(ta->m_temp_points[i+0]);
				ta->m_final_points.push_back(ta->m_temp_points[i+1]);
				ta->m_final_points.push_back(ta->m_temp_points[i+2]);
			}
			break;
		...
		case GL_TRIANGLE_STRIP:
			...
			// append strip points
			ta->m_final_points.append(ta->m_temp_points);
			break;
		...
		case GL_TRIANGLE_FAN:
			...
			if(ta->m_final_points.size() > 0)
			{
				ta->m_final_points.push_back(ta->m_temp_points[1]);
			}

			// convert triangle fan to triangle strip
			if(ta->m_temp_points.size() >= 4)
			{
				ta->m_final_points.push_back(ta->m_temp_points[1]);
				ta->m_final_points.push_back(ta->m_temp_points[0]);
				ta->m_final_points.push_back(ta->m_temp_points[2]);
				ta->m_final_points.push_back(ta->m_temp_points[3]);
			}
			for(int i = 4; i < ta->m_temp_points.size(); i++)
			{
				ta->m_final_points.push_back(ta->m_temp_points[0]);
				ta->m_final_points.push_back(ta->m_temp_points[i]);
			}
			break;
	...
	static void callback_end_tris(tesselator_accepter* ta)
	{
		//printf("callback_end\n");
		//for(int i = 0; i < ta->m_temp_points.size(); i++)
		//	printf("point (%.1f, %.1f)\n", ta->m_temp_points[i].m_x/20, ta->m_temp_points[i].m_y/20);

		unsigned short base_index = ta->m_final_points.size();
		...
		case GL_TRIANGLES:
			...
			for(int i = 0; i < ta->m_temp_points.size(); i++)
			{
				ta->m_final_indexes.push_back(base_index + i);
			}
			break;
		
		case GL_TRIANGLE_STRIP:
			// convert strip to indexed triangles
			ta->m_final_points.append(ta->m_temp_points);

			for(int i = 2; i < ta->m_temp_points.size(); i++)
			{
				ta->m_final_indexes.push_back(base_index + i-2);
				ta->m_final_indexes.push_back(base_index + i-1);
				ta->m_final_indexes.push_back(base_index + i);
			}
			break;
		...
		case GL_TRIANGLE_FAN:
			...
			for(int i = 2; i < ta->m_temp_points.size(); i++)
			{
				ta->m_final_indexes.push_back(base_index + 0);
				ta->m_final_indexes.push_back(base_index + i - 1);
				ta->m_final_indexes.push_back(base_index + i);
			}
			break;
...
struct final_shape
{
	...
	struct segment
	{
		...
		void reverse()
		{
			for(int i = 0; i < m_points.size()/2; i++)
			{
				...
				m_points[i] = m_points[m_points.size() - 1 - i];
				m_points[m_points.size() - 1 - i] = temp;
			}
		}
	};
	...
	void init(float error_tolerance, int style, bool contour)
	{
		m_error_tolerance = (error_tolerance > 0 ? error_tolerance : 1.0f);
		m_style = style;
		m_contour = contour;
	...
	//void new_contour()
	//{
	//	m_points.push_back(array<point>());
	//}

	void flush(mesh_set* m, tesselator_accepter& ta)
	{
		ta.init();
		ta.begin_shape();
		ta.begin_contour();
		...
		for(int i = 0; i < m_segments.size(); i++)
		{
			...
			//printf("   (%.2f, %.2f) -> (%.2f, %.2f) %d pts\n", seg.m_points.front().m_x/20, seg.m_points.front().m_y/20, seg.m_points.back().m_x/20, seg.m_points.back().m_y/20, seg.m_points.size());

			int last = seg.m_points.size();
			
			// skip last closing point (identical to 1st) otherwise tesselator can create an invisible triangle
			if(!m_contour && seg.m_points.front() == seg.m_points.back())
				last = seg.m_points.size() - 1;

			for(int j = 0; j < last; j++)
			{
				ta.add_point(seg.m_points[j]);
			}

			if(seg.m_points.front() == seg.m_points.back())
			{
				ta.end_contour();
				ta.begin_contour();
			}
		...
		ta.end_contour();
		ta.end_shape(m, m_style);
		m_segments.clear();
		//new_contour();
	}

	bool merge_segment(segment* s)
	{
		for(int i = 0; i < m_segments.size(); i++)
		{
			segment& seg = m_segments[i];
			if(seg.m_points.size() == 0 || s == &seg)
				continue;

			if(seg.m_points.front() == s->m_points.back())
			{
				s->m_points.pop_back();
				s->m_points.append(seg.m_points);
				seg.m_points = s->m_points;
				return true;
			}
			else if(seg.m_points.back() == s->m_points.back())
			{
				// 2 paths have same ends, insert in reverse order
				s->m_points.pop_back();
				s->reverse();
				seg.m_points.append(s->m_points);
				return true;
			}
			else if(seg.m_points.back() == s->m_points.front())
			{
				seg.m_points.pop_back();
				seg.m_points.append(s->m_points);
				return true;
			}
			else if(seg.m_points.front() == s->m_points.front())
			{
				s->reverse();
				s->m_points.pop_back();
				s->m_points.append(seg.m_points);
	...
	void end_path()
	{
		if(m_segment.m_points.size() == 0)
			return;

		// copy segment into all points
		if(m_contour)
		{
		...
		{
			if(!merge_segment(&m_segment))
			{
				m_segments.push_back(m_segment);
			}
		}

		m_segment.m_points.clear();
	}

	void add_line_segment(float ax, float ay)
	{
		//printf("line_segment (%.1f, %.1f)\n", ax/20, ay/20);

		m_segment.m_points.push_back(point(ax, ay));
		m_last = m_segment.m_points.back();
	}

	void add_curve(float p0x, float p0y, float p1x, float p1y, float p2x, float p2y)
	// Recursive routine to generate bezier curve within tolerance.
	{
#ifndef NDEBUG
		static int	recursion_count = 0;
		...
		{
			// Error is too large; subdivide.
			add_curve(p0x, p0y, (p0x + p1x) * 0.5f, (p0y + p1y) * 0.5f, qx, qy);
			add_curve(qx, qy, (p1x + p2x) * 0.5f, (p1y + p2y) * 0.5f, p2x, p2y);
		}
...
void execute_tesselate(mesh_set* m, const shape_character_def* scd, float error_tolerance)
{
	const bool use_strips = false;
	tesselator_accepter ta_fill(use_strips, false, false);
	tesselator_accepter ta_line(use_strips, true, false);

	const array<path>& paths = scd->get_paths();

	array<final_shape> fill_paths;
	fill_paths.resize(imax(scd->get_fill_styles().size(), 1));
	for(int i = 0; i < fill_paths.size(); i++)
	{
		fill_paths[i].init(error_tolerance, i, false);
	}
	
	array<final_shape> line_paths;
	line_paths.resize(scd->get_line_styles().size());
	for(int i = 0; i < line_paths.size(); i++)
	{
		line_paths[i].init(error_tolerance, i, true);
	}
	
	m->new_layer();

	const int start = 0;
	const int end = paths.size();
	...
	for (int i = start; i < end; i++)
	{
		...
		//printf("path %d (%d edges) fill0=%d fill1=%d line=%d\n", i, p.m_edges.size(), p.m_fill0 - 1, p.m_fill1 - 1, p.m_line - 1);

		bool flush_fills = false;
		bool flush_lines = false;
		bool new_layer = false;
		if(i + 1 < end && paths[i+1].m_new_shape)
		{
			new_layer = flush_fills = flush_lines = true;
		}
		else if(i + 1 == end)
		{
			flush_fills = flush_lines = true;
		}

		for(int s = 0; s < 3; s++)
		{
			const bool contour = s == 2;
			int style = 0;
			if(s == 0)
			{
				style = p.m_fill0 - 1;
			...
			final_shape& fp = contour ? line_paths[style] : fill_paths[style];
			fp.begin_path(p.m_ax, p.m_ay);
			
			for (int k = 0; k < p.m_edges.size(); k++)
			{
				const edge& e = p.m_edges[k];

				if (e.m_cx == e.m_ax && e.m_cy == e.m_ay)
				{
					// Early-out for degenerate straight segments.
					fp.add_line_segment(e.m_ax, e.m_ay);
				}
				else
				{
					// Subdivide, and add line segments...
					fp.add_curve(fp.m_last.m_x, fp.m_last.m_y, e.m_cx, e.m_cy, e.m_ax, e.m_ay);
				}
			}
...
triangle_intersector::triangle_intersector()
{
	m_accepter = swfnew tesselator_accepter(true, false, true);
}

triangle_intersector::~triangle_intersector()
{
	swfdelete (m_accepter);
}

inline float crossp(point* p)
{
	point v1(p[1].m_x - p[0].m_x, p[1].m_y - p[0].m_y);
	point v2(p[2].m_x - p[0].m_x, p[2].m_y - p[0].m_y);
	return -v1.m_x * v2.m_y + v1.m_y * v2.m_x;
}

void triangle_intersector::process(point* t1, point* t2, array<point>& result)
{
	m_accepter->init();
	m_accepter->begin_shape();

	// ensure same winding: cross products to view if normal in same direction
	float up1 = crossp(t1);
	float up2 = crossp(t2);
	bool winding = up1 * up2 >= 0.0f;

	m_accepter->begin_contour();
	for(int i = 0; i < 3; i++)
	{
		m_accepter->add_point(t1[i]);
	}
	m_accepter->end_contour();
	...
	array<unsigned short> result_indexes;
	m_accepter->end_shape(result, result_indexes);
}
