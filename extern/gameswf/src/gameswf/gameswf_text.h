...
namespace gameswf
{
	...
	struct text_style
	{
		...
		void	resolve_font(movie_definition_sub* root_def)
		{
			if (m_font == NULL && m_has_font)
			{
				assert(m_font_id >= 0);
	...
	// be changed at runtime (by script or host).
	{
		...
		// <p>...</p> -- paragraph
		// <tab> -- insert tab
		// <img src='test.png' [width='10' height='10']> -- insert an image
	...
	struct edit_text_character : public character
	{
		...
		float m_xglobal;
		float m_yglobal;
		int m_last_line_start_record;
		int m_last_space_record;
		int m_last_space_glyph;
		...
		void	set_text_value(const tu_string& new_text, bool html = false);
		...
		void	format_text(bool html = false);
		void	append_text(const tu_string& str, text_attributes& attributes, bool html);
		void	append_image(const tu_string& src, int width, int height);
	...
	private:
		...
		void	set_text(const tu_string& new_text, bool html = false);
