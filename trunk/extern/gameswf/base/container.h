...
class shared_array
{
	...
	public:
		...
		shared_array ( int size_hint ) : m_buffer ( 0 ), m_size ( 0 ), m_buffer_size ( 0 )
		{
			resize ( size_hint );
		}
...
class hash
{
	...
	public:
		hash() : m_table ( NULL ) { }
		hash ( int size_hint ) : m_table ( NULL )
		{
		...
		struct entry
		{
			...
			bool is_empty() const
			{
				return m_next_in_chain == -2;
			}
			bool is_end_of_chain() const
			{
				return m_next_in_chain == -1;
			}
			bool is_tombstone() const
			{
				return m_hash_value == TOMBSTONE_HASH;
			}
		...
		class const_iterator
		{
			public:
				T	get_key() const
				{
					return m_hash->E ( m_index ).first;
				}
				U	get_value() const
				{
					return m_hash->E ( m_index ).second;
				}
		...
		const_iterator	begin() const
		{
			return const_cast<hash *> ( this )->begin();
		}
		const_iterator	end() const
		{
			return const_cast<hash *> ( this )->end();
		}
...
class tu_string
{
	public:
		...
		exported_module int	utf8_length() const
		{
			return utf8_char_count ( get_buffer(), length() );
		}
...
class tu_stringi
{
	public:
		...
		void	operator= ( const tu_stringi &str )
		{
		...
		tu_stringi	operator+ ( const char *str ) const
		{
			return tu_stringi ( m_string + str );
		}

		// The special stuff.
		tu_string &to_tu_string()
		{
			return m_string;
		}
