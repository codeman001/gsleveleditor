...
namespace gameswf
{
	struct button_character_instance : public character
	{
		...
		virtual bool	on_event ( const event_id &id )
		{
			...
			else
			{
				...
				else if ( id.m_id == event_id::PRESS )
				{
				...
				else if ( id.m_id == event_id::DRAG_OUT )
				{
					c |= ( button_action::OVER_DOWN_TO_OUT_DOWN );
				}

				else if ( id.m_id == event_id::DRAG_OVER )
				{
					c |= ( button_action::OUT_DOWN_TO_OVER_DOWN );
				}

				else if ( id.m_id == event_id::RELEASE_OUTSIDE )
				{
	...
	void	button_action::read ( stream *in, int tag_type )
	{
		...
		action_buffer	*a = new action_buffer ( -1 );
		a->read ( in );
