...
namespace gameswf
{
	...
	struct character_def : public as_object_interface
	{
			...
			virtual void	csm_textsetting ( stream *in, int tag_type )
			{
				assert ( 0 );
			};
	...
	struct character : public as_object
	{
			...
			virtual void	clear_display_objects()
			{
				/* assert(0); */
			}
			virtual void	remove_display_object ( character *ch )
			{
				assert ( 0 );
			}
			...
			// ** dmsovetov
			virtual display_list	*get_display_list ( void )
			{
				return NULL;
			}
			...
			{
				m_parent = parent;    // for extern movie
			}
			int	get_depth() const
			{
				return m_depth;
			...
			Uint16	get_clip_depth() const
			{
				return m_clip_depth;
			}
			void	set_clip_depth ( Uint16 d )
			{
				m_clip_depth = d;
			}
			Uint8   get_blend_mode() const
			{
				return m_blend_mode;
			}
			void    set_blend_mode ( Uint8 d )
			{
				m_blend_mode = d;
			}
			...
			virtual character	*get_root_movie() const
			{
