...
#define GLOBAL_REGISTER_COUNT 4
	...
	struct vm_stack : private array<as_value>
	{
			...
			as_value	&top ( int dist )
			{
				return ( *this ) [m_stack_size - 1 - dist];
			}
			as_value	&bottom ( int index )
			{
				return ( *this ) [index];
			}
			inline int	get_top_index() const
			{
				return m_stack_size - 1;
			}
	...
	struct as_environment : public vm_stack
	{
			...
			int	get_local_frame_top() const
			{
				return m_local_frames.size();
			}
			void	set_local_frame_top ( int t )
			{
