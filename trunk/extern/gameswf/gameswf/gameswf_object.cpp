...
namespace gameswf
{
	...
	void	as_object_registerclass ( const fn_call &fn )
	{
		...
		if ( fn.nargs == 2 && fn.env->get_target() != NULL )
		{
			...
			if ( def )
			{
				...
				{
					const as_value &a0 = fn.arg ( 0 );
					const as_value &a1 = fn.arg ( 1 );
					log_error ( "can't find class function '%s'('%s')\n", a0.to_string(), a1.to_string() );
				}
			}
	...
	// add 'this' as listener of as_event object
	{
		if ( fn.nargs >= 2 )
		{
			as_value &e = fn.arg ( 0 );
			as_value &m = fn.arg ( 1 );
			call_method ( m, fn.env, fn.this_ptr, 0, 0 );
			// find event handler
			assert ( fn.this_ptr );
			as_value val;
	...
	// retrieves members & print them
	{
		      ...
		      it != m_members.end(); ++it )
		{
			...
			else if ( val.is_function() )
			{
				...
				else if ( cast_to<as_3_function> ( val.to_object() ) )
				{
	...
	void as_object::create_traits ( abc_def *abc, instance_info *ii )
	{
		if ( !ii )
		{
			return;
		...
		if ( const char *superclass = abc->get_multiname ( ii->m_super_name ) )
		{
			instance_info *ii = abc->get_instance_info ( superclass );
			create_traits ( abc, ii );
		}
				...
				case traits_info::Trait_Method:
					{
						...
						val.set_as_object ( abc->m_method[index].get() );
						break;
					}
