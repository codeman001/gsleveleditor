...
// ** save_texture
// ** dm
void save_texture ( const char *fileName, unsigned int *data, int w, int h, int c )
{
	FILE	*file;
	int		image_size = 0;
	byte tga_header_a[12]   = { 0, 0, 2, 0, 0, 0, 0, 0, 0, 0, 0, 0 };
	byte tga_info_header[6] = { 0, 0, 0, 0, 0, 0 };
	file = fopen ( fileName, "wb" );
	fwrite ( tga_header_a, 1, sizeof ( tga_header_a ), file );
	tga_info_header[0] = w  % 256;
	tga_info_header[1] = w  / 256;
	tga_info_header[2] = h % 256;
	tga_info_header[3] = h / 256;
	tga_info_header[4] = c * 8;
	tga_info_header[5] = 0;
...
// resampling to create the dst image.
{
	...
	for ( int j = 0; j < src_height; j++ )
	{
		...
		{
			Uint8 *dst = &rescaled[j * dst_width * bytes_per_pixel + i * bytes_per_pixel];
			Uint8 *src = &src_data[j * src_width * bytes_per_pixel + i * bytes_per_pixel];
			*dst++ = *src++;
			*dst++ = *src++;
			*dst++ = *src++;
	...
	/*
		#define DEBUG_WRITE_TEXTURES_TO_PPM
		#ifdef DEBUG_WRITE_TEXTURES_TO_PPM
			static int s_image_sequence = 0;
			char temp[256];
			sprintf(temp, "image%d_r.tga", s_image_sequence );
			save_texture( temp, ( unsigned int* )rescaled, dst_width, dst_height, bytes_per_pixel );
			sprintf(temp, "image%d.tga", s_image_sequence );
			save_texture( temp, ( unsigned int* )src_data, src_width, src_height, bytes_per_pixel );
			...
			case image::image_base::RGBA:
				{
					...
					{
						if ( w < m_suspended_image->m_width )
						{
							w = w * 2;
						}

						if ( h < m_suspended_image->m_height )
						{
							h = h * 2;
						}
