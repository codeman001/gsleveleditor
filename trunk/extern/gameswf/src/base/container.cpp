...
void tu_string::append_wide_char(uint16 c)
{
	char buf[8];
	int index = 0;
	encode_unicode_character(buf, &index, (uint32) c);
	buf[index] = 0;
...
void tu_string::append_wide_char(uint32 c)
{
	char buf[8];
	int index = 0;
	encode_unicode_character(buf, &index, c);
	buf[index] = 0;
...
void	tu_string::resize(int new_size)
{
	...
	if (using_heap() == false)
	{
		if (new_size < GAMESWF_STRING_INTERNAL_CAPACITY)
		{
		...
		else
		{
			...
			// round up.
			capacity = (capacity + GAMESWF_STRING_INTERNAL_CAPACITY) & ~GAMESWF_STRING_INTERNAL_CAPACITY;
	...
	else
	{
		...
		if (new_size < GAMESWF_STRING_INTERNAL_CAPACITY)
		{
			...
			m_local.m_size = (char) (new_size + 1);
			strncpy(m_local.m_buffer, old_buffer, GAMESWF_STRING_INTERNAL_CAPACITY);
...
/*static*/ void	encode_utf8_from_wchar_generic(tu_string* result, const char_type* wstr)
{
	...
	for (;;)
	{
		Uint32	uc = *in++;
		offset = 0;
		encode_unicode_character(dummy, &offset, uc);
		bytes_needed += offset;
	...
	for (;;)
	{
		...
		Uint32	uc = *in++;
		encode_unicode_character(out, &offset, uc);
...
uint32	tu_string::utf8_char_at(int index) const
{
	...
	do
	{
		c = decode_next_unicode_character(&buf);
		index--;
...
tu_string	tu_string::utf8_to_upper() const
{
	...
	for (;;)
	{
		uint32 c = decode_next_unicode_character(&buf);
...
/*static*/ int	tu_string::utf8_char_count(const char* buf, int buflen)
{
	...
	while (p - buf < buflen)
	{
		uint32	c = decode_next_unicode_character(&p);
		if (c == 0)
		{
			break;
		}
...
// 			printf("duplicate entry %x, prev swfnew index %d, current array index %d\n",
// 				   numbers[i],
// 				   it->second,
// 				   i);
		}
