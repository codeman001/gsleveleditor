...
// Simple bounding box class.
{
	...
	index_box(coord_t min_max_x, coord_t min_max_y)
		:
		m_min(min_max_x, min_max_y),
		m_max(min_max_x, min_max_y)
	{
	}
	...
	const index_point<coord_t>& get_min() const { return m_min; }
	const index_point<coord_t>& get_max() const { return m_max; }
	
	coord_t	get_width() const { return m_max.x - m_min.x; }
	coord_t	get_height() const { return m_max.y - m_min.y; }
	...
	{
		if (loc.x >= m_min.x && loc.x <= m_max.x && loc.y >= m_min.y && loc.y <= m_max.y)
		{
			return true;
		}
		else
...
//data:
	index_point<coord_t>	m_min;
	index_point<coord_t>	m_max;
};
...
// Grid-based container for points.
{
		...
		m_y_cells(y_cells)
	{
		...
		assert(bound.m_min.x <= bound.m_max.x);
		assert(bound.m_min.y <= bound.m_max.y);

		// Allocate the grid.
		m_grid = swfnew grid_entry_t*[x_cells * y_cells];
		...
		m_bound(bound)
	{
		...
		// Allocate the grid.
		m_grid = swfnew grid_entry_t*[m_x_cells * m_y_cells];
	...
	~grid_index_point()
	{
		...
		swfdelete_array (m_grid, m_x_cells * m_y_cells);
	}
	...
	struct iterator
	{
		...
		// Point at next element in the iteration.
		{
			...
			m_current_cell_x++;
			while (m_current_cell_y <= m_query_cells.m_max.y)
			{
				for (;;)
				{
					if (m_current_cell_x > m_query_cells.m_max.x)
					{
						break;
					}
				...
				m_current_cell_x = m_query_cells.m_min.x;
				m_current_cell_y++;
			}

			assert(m_current_cell_x == m_query_cells.m_min.x);
			assert(m_current_cell_y == m_query_cells.m_max.y + 1);
	...
	iterator	begin(const box_t& q)
	{
		...
		it.m_index = this;
		it.m_query = q;
		it.m_query_cells.m_min = get_containing_cell_clamped(q.m_min);
		it.m_query_cells.m_max = get_containing_cell_clamped(q.m_max);

		assert(it.m_query_cells.m_min.x <= it.m_query_cells.m_max.x);
		assert(it.m_query_cells.m_min.y <= it.m_query_cells.m_max.y);

		it.m_current_cell_x = it.m_query_cells.m_min.x;
		it.m_current_cell_y = it.m_query_cells.m_min.y;
	...
	// our index.
	{
		...
		grid_entry_t*	new_entry = swfnew grid_entry_t;
...
private:
	...
	{
		index_point<int>	ip;
		ip.x = int(((p.x - m_bound.m_min.x) * coord_t(m_x_cells)) / (m_bound.m_max.x - m_bound.m_min.x));
		ip.y = int(((p.y - m_bound.m_min.y) * coord_t(m_y_cells)) / (m_bound.m_max.y - m_bound.m_min.y));
...
// Grid-based container for boxes.
{
		...
		m_query_id(0)
	{
		...
		// Allocate the grid.
		m_grid = swfnew array<grid_entry_t*>[x_cells * y_cells];
	}
		...
		m_query_id(0)
	{
		...
		assert(bound.m_min.x <= bound.m_max.x);
		assert(bound.m_min.y <= bound.m_max.y);
		m_grid = swfnew array<grid_entry_t*>[m_x_cells * m_y_cells];
	}
	...
	// Removes the entry from the index, and deletes it.
	{
		...
		for (int iy = ib.m_min.y; iy <= ib.m_max.y; iy++)
		{
			for (int ix = ib.m_min.x; ix <= ib.m_max.x; ix++)
			{
...
private:
	...
	// Get the indices of the cell that contains the given point.
	{
		index_box<int>	ib(get_containing_cell_clamped(p.m_min), get_containing_cell_clamped(p.m_max));
		return ib;
	}
