...
{
	void*	mem = (void*) swfnew char[count * size];
	memset(mem, 0, count * size);
	return mem;
}


inline void	my_cfree(void* mem, int size)
{
	swfdelete_primitives ((char*) mem, size);
}


void	get_row(Uint8* row, gameswf::image_rgb* image, int x0, int xsize, int y)
...
void	get_row(Uint8* row, gameswf::image_rgba* image, int x0, int xsize, int y)
...
void	get_column(Uint8* column, gameswf::image_rgb* image, int x)
...
void	get_column(Uint8* column, gameswf::image_rgba* image, int x)
...
void	put_pixel(gameswf::image_rgb* image, int x, int y, float r, float g, float b)
// Clamp {r, g, b} to [0,255], and write pixel data to the given image
// at (x, y).
{
	static gameswf::image_rgb*	im = NULL;
	static int	yy = -1;
	static Uint8*	p = NULL;
...
void	put_pixel(gameswf::image_rgba* image, int x, int y, float r, float g, float b, float a)
// Clamp {r, g, b, a} to [0,255], and write pixel data to the given image
// at (x, y).
{
	static gameswf::image_rgba*	im = NULL;
	static int	yy = -1;
	static Uint8*	p = NULL;
...
// portion of the output image.  Coordinates are *inclusive*.
{
	...
	image_rgb*	tmp;		/* intermediate image */
	...
	tmp = create_rgb(out_width, in_window_h);
	...
	my_cfree(raster, tmp->m_height * 3);
...
// Same as above, but with an alpha channel.
{
	...
	image_rgba*	tmp;		/* intermediate image */
	float	xscale, yscale;		/* zoom scale factors */
	int i, j, k;			/* loop variables */
	int n;				/* pixel number */
	float center; int left, right;	/* filter calculation variables */
	...
	/* create intermediate image to hold horizontal zoom */
	tmp = create_rgba(out_width, in_window_h);
	xscale = (float) (out_width - 1) / in_width;
	yscale = (float) (out_height - 1) / in_height;
	...
	my_cfree(raster, tmp->m_height * 4);
