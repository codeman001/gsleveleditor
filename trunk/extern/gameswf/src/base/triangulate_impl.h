...
// Helper.  Return the swfnew value of index, for the case of verts [duped_v0]
...
struct poly
{
	...
	~poly()
	{
		swfdelete (m_edge_index);
		m_edge_index = NULL;

		swfdelete (m_reflex_point_index);
		m_reflex_point_index = NULL;
	}
...
// Link the specified vert into our loop.
{
	...
	{
		// We have a loop.  Link the swfnew vert in, behind the
		// first vert.
		...
		{
			// v is to the left of vl; it's the swfnew leftmost vert
			m_leftmost_vert = vert_index;
		}
	}
...
// verts being shifted up.
{
	...
	if (m_edge_index)
	{
		...
		bound.m_min.x = sorted_verts[v0].m_v.x;
...
// owner, update our m_leftmost_vert and our m_vert_count.
{
	...
	do
	{
		...
		// Insert swfnew edge into the edge index.
...
// intersecting edges (when constructing bridges to join polys).
{
	...
	m_edge_index = swfnew grid_index_box<coord_t, int>(bound_of_all_verts, x_cells, y_cells);
...
// verts within a potential ear.
{
	...
	m_reflex_point_index = swfnew grid_index_point<coord_t, int>(reflex_bound, x_cells, y_cells);
...
//code:
	...
	~poly_env()
	{
		// delete the polys that got swfnew'd during init()
...
// and component polys.
{
	...
	for (int i = 0; i < path_count; i++)
	{
		...
		poly<coord_t>*	p = swfnew poly<coord_t>;
		m_polys.push_back(p);
	...
	// of two verts.
	array<int>	vert_remap;	// vert_remap[i] == swfnew index of original vert[i]
...
// between the two given verts.
{
	...
	if (m_sorted_verts[vert_on_main_poly].m_v == m_sorted_verts[vert_on_sub_poly].m_v)
	{
		...
		// don't actually need to make a bridge with swfnew
	...
	// Fixup the old indices to account for the swfnew dupes.
...
// Duplicate the two indexed verts, remapping polys & verts as necessary.
{
	...
	// Slower, clearer code to insert the two swfnew verts.  This
	...
	// Faster, more obfuscated code to insert the two swfnew verts.
	//
	...
	// slower!  (VC7, Win2K.)  memmove() must be magical?
	{
		...
		// Insert the swfnew duplicate verts.
