...
#else // GAMESWF_LINK_GLF

#define USE_CUSTOM_FILESYSTEM	1

void* fs_open_func(const char* filename, const char* mode);
int fs_read_func(void* dst, int bytes, void* appdata);
int fs_write_func(const void* src, int bytes, void* appdata);
int fs_seek_func(int pos, void *appdata);
int fs_seek_to_end_func(void *appdata);
int fs_tell_func(const void *appdata);
bool fs_get_eof_func(void *appdata);
int fs_close_func(void *appdata);

#endif // GAMESWF_LINK_GLF

namespace gameswf
{
#if GAMESWF_LINK_GLF
static int glf_read_func(void* dst, int bytes, void* appdata)
{
	glf::IOStream* stream = (glf::IOStream*)appdata;
	return stream->Read(dst, bytes);
}
static int glf_write_func(const void* src, int bytes, void* appdata)
{
	glf::IOStream* stream = (glf::IOStream*)appdata;
	return 0;
}
static int glf_seek_func(int pos, void *appdata)
{
	glf::IOStream* stream = (glf::IOStream*)appdata;
	stream->Seek(pos, glf::ios::beg);
	return stream->Tell() == pos ? 0 : gameswf::TU_FILE_SEEK_ERROR;
}
static int glf_seek_to_end_func(void *appdata)
{
	glf::IOStream* stream = (glf::IOStream*)appdata;
	stream->Seek(0, glf::ios::end);
	return 0;
}
static int glf_tell_func(const void *appdata)
{
	glf::IOStream* stream = (glf::IOStream*)appdata;
	return stream->Tell();
}
static bool glf_get_eof_func(void *appdata)
{
	glf::IOStream* stream = (glf::IOStream*)appdata;
	return stream->IsEof();
}
static int glf_close_func(void *appdata)
{
	glf::IOStream* stream = (glf::IOStream*)appdata;
	swfdelete (stream);
	return 0;
...
static glf::IOStream* (*s_file_opener)(const char* filename, const char* mode) = NULL;

void set_file_opener(glf::IOStream* (*callback)(const char* filename, const char* mode))
{
	s_file_opener = callback;
}
#endif // GAMESWF_LINK_GLF
...
struct filebuf
{
	...
	unsigned char*	get_cursor()
	{
		if(m_read_only)
		{
			const membuf& buf = (const membuf&)m_;
			return ((unsigned char*) buf.data()) + m_position;
		}
		else
		{
...
#if GAMESWF_LINK_GLF
	assert(s_file_opener != NULL);
	glf::IOStream* stream = s_file_opener(name, mode);
	if(stream && stream->IsOpened())
	{
		m_data = stream;
		m_read = glf_read_func;
		m_write = glf_write_func;
		m_seek = glf_seek_func;
		m_seek_to_end = glf_seek_to_end_func;
		m_tell = glf_tell_func;
		m_get_eof = glf_get_eof_func;
		m_close = glf_close_func;
		m_error = TU_FILE_NO_ERROR;
	}
	else if(stream)
	{
...
#elif USE_CUSTOM_FILESYSTEM
	m_open = fs_open_func;

	m_data = m_open(name, mode);
	if(m_data)
	{
		m_read = fs_read_func;
		m_write = fs_write_func;
		m_seek = fs_seek_func;
		m_seek_to_end = fs_seek_to_end_func;
		m_tell = fs_tell_func;
		m_get_eof = fs_get_eof_func;
		m_close = fs_close_func;
		m_error = TU_FILE_NO_ERROR;
	}
#else
...
{
	//	m_data = swfnew filebuf;
	//	should actually be
	m_data = swfnew filebuf();	// thanks to Eli Curtz 
...
{
	//	m_data = swfnew membuf(data, size);
	//	should actually be
	m_data = swfnew filebuf(size, data);	// thanks to Eli Curtz 
...
void	tu_file::read_fully(membuf* dst, int bytes)
// Copy all contents of *this into *dst.
{
	int remain = bytes == -1 ? dst->size() : bytes;
	int offset = 0;

	while (get_eof() == false && remain > 0)
	{
		// assert enough room.
		assert(offset + remain <= dst->size());

		int bytes_read = read_bytes(((char*) dst->data()) + offset, remain);
		offset += bytes_read;
		remain -= bytes_read;

		if (get_error() || bytes_read <= 0)
		{
			break;
		}
	}
...
int	buffered_read(void* dst, int bytes, void* appdata)
// Return number of bytes actually read.
{
	buffered_file_impl*	buf = (buffered_file_impl*) appdata;
	int read_count = 0;
	char* out = (char*)dst;
	...
	while(bytes > 0)
	{
		if(buf->available == 0)
		{
			buf->offset = buf->file->get_position();
			buf->available = buf->file->read_bytes(buf->data, sizeof(buf->data));
			buf->pos = 0;
		}
		...
		int count = imin(bytes, buf->available);
	...
	return read_count;
	//return buf->file->read_bytes(dst, bytes);
}

int	buffered_write(const void* src, int bytes, void* appdata)
...
int	buffered_seek(int pos, void* appdata)
// Try to go to pos.  Return actual pos.
{
	buffered_file_impl*	buf = (buffered_file_impl*) appdata;
	int diff = pos - (buf->offset + buf->pos);
	if(pos < buf->offset || buf->pos + diff >= sizeof(buf->data))
	{
		// out of buffered data
		buf->file->set_position(pos);
		buf->offset = buf->file->get_position();
		buf->available = 0;
		buf->pos = 0;
	}
	...
	return buf->offset + buf->pos;
}

int	buffered_seek_to_end(void* appdata)
// Return 0 on success, TU_FILE_SEEK_ERROR on failure.
{
	buffered_file_impl*	buf = (buffered_file_impl*) appdata;
	buf->file->go_to_end();
...
int	buffered_tell(const void* appdata)
{
	buffered_file_impl*	buf = (buffered_file_impl*) appdata;
	return buf->offset + buf->pos;
}

bool buffered_get_eof(void* appdata)
// Return true if we're at EOF.
{
	buffered_file_impl*	buf = (buffered_file_impl*) appdata;
	return buf->file->get_eof() && buf->available == 0;
}

int buffered_close(void *appdata)
// Return 0 on success, or TU_FILE_CLOSE_ERROR on failure.
{
	buffered_file_impl*	buf = (buffered_file_impl*) appdata;
	if(buf != NULL)
...
tu_file* get_buffered_file(tu_file* in)
{
	assert(in);

	buffered_file_impl*	buf = swfnew buffered_file_impl(in);
	return swfnew tu_file(
		buf,
		buffered_read,
		buffered_write,
		buffered_seek,
		buffered_seek_to_end,
		buffered_tell,
		buffered_get_eof,
		buffered_close);
}
