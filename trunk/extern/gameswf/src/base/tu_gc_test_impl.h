...
gc_ptr<cons> build_list_of_numbers(int low, int high) {
	if (low <= high) {
		return swfnew cons(low, build_list_of_numbers(low + 1, high).get());
	} else {
		return NULL;
	}
}
...
gc_ptr<cons> build_cycle(int size) {
	gc_ptr<cons> head = swfnew cons(-1, NULL);
	gc_ptr<cons> tail = head;
	for (int i = 0; i < size - 1; i++) {
		head = swfnew cons(i, head.get());
	}
...
void test_multiple_inheritance() {
	...
	gc_ptr<actor> ship = swfnew spaceship;
	gc_ptr<tractor_beam> beam = swfnew tractor_beam;
...
void test_gc_container() {
	...
	gc_ptr<bag> b = swfnew bag;
	b->m_ptrs.push_back(swfnew bag);
	b->m_ptrs.push_back(swfnew bag);
	b->m_ptrs.push_back(swfnew bag);
	b->m_ptrs[1]->m_ptrs.push_back(swfnew bag);
