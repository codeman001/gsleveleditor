...
namespace profiler
{
	...
	struct scope_event
	{
		scope_event(const char* name) { begin_event(name); };
		~scope_event() { end_event(); };
	};
}; // namespace profiler

#if GAMESWF_ENABLE_PROFILER
	#define GAMESWF_PROFILER_BEGIN_EVENT(_e)	gameswf::profiler::begin_event(_e)
	#define GAMESWF_PROFILER_END_EVENT()		gameswf::profiler::end_event()
	#define GAMESWF_PROFILER_SCOPE_EVENT(_e)	gameswf::profiler::scope_event __scope_event_object(_e)
#else
	#define GAMESWF_PROFILER_BEGIN_EVENT(_e)
	#define GAMESWF_PROFILER_END_EVENT()
	#define GAMESWF_PROFILER_SCOPE_EVENT(_e)
#endif
