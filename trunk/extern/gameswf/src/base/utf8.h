...
namespace gameswf
{
	...
	// Helper function to format string correctly:
	// Replace string occurrences of " !", " ?", " :", " ;" by a non-breakable character to prevent wrapping.
	// Also, replace auto_line_feed (ie: pipe '|') character by AUTO_LINE_FEED, if specified.
	void format_utf_text(const char* in, char* out, int auto_line_feed = -1);
};
