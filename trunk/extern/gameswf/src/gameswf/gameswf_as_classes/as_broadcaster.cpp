...
namespace gameswf
{
	...
	void	as_broadcaster_initialize(const fn_call& fn)
	{
		if (fn.nargs == 1)
		{
			...
			if (obj)
			{
				obj->set_member("_listeners", swfnew as_listener(fn.get_player()));
	...
	{
		as_object* bc = swfnew as_object(player);
	...
	void	as_listener::broadcast(const fn_call& fn)
	{
		...
		as_value this_in((as_object*)NULL);
		m_listeners.notify(event_name, 
			fn_call(NULL, this_in, fn.env, fn.nargs - 1, fn.env->get_top_index(), event_name.c_str()));
		...
		while (m_suspended_event.size() > 0)
		{
			...
			as_value this_in((as_object*)NULL);
			m_listeners.notify(event_name, 
				fn_call(NULL, this_in, fn.env, arg.size() - 1, fn.env->get_top_index(), event_name.c_str()));
