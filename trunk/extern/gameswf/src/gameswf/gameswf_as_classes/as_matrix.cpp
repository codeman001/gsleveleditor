...
namespace gameswf
{
	...
	void	as_global_matrix_ctor(const fn_call& fn)
	{
		...
		matrix = swfnew as_matrix(fn.get_player());
		switch(fn.nargs)
		{
		default:
	...
	void as_matrix_clone(const fn_call& fn)
	{
		...
		result = swfnew as_matrix(fn.get_player());
	...
	void as_matrix_transformPoint(const fn_call& fn)
	{
		...
		if (point)
		{
			...
			result = swfnew as_point(fn.get_player(), .0f, 0.0f);
			matrix->m_matrix.transform( &result->m_point, point->m_point);
			fn.result->set_as_object(result.get_ptr());
		}
	}

	as_matrix::as_matrix(player* player, const matrix* mat) :
		as_object(player)
	{
		if(mat != NULL)
			m_matrix = *mat;
