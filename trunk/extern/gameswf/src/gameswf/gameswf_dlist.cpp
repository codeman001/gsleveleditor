...
namespace gameswf
{
	...
	{
		stringi_pointer key(name);
		character* ch = NULL;
		m_character_map.get(key, &ch);
		return ch;
	}
	...
	// Removing the character at get_display_object(index).
	{
		...
		IF_VERBOSE_DLIST(const char* name = di.m_character->get_name(); log_msg("dl::remove(%d, '%s')\n", index, name));
		...
		smart_ptr<character> ch = di.m_character.get_ptr();

		stringi_pointer key(di.m_character->m_name.to_tu_stringi());
		m_character_map.erase(key);
		di.set_character(NULL);
		m_display_object_array.remove(index);

		if(ch->get_ref_count() <= 2) // 1 for smart_ptr, 1 for player heap
		{
			ch->get_player()->notify_unused_instance(ch.get_ptr());
		}
	}
		...
		Uint16 clip_depth)
	{
		...
		IF_VERBOSE_DLIST(const char* name = ch->get_name(); log_msg("dl::add(%d, '%s')\n", depth, name));
		...
		else
		{
			...
			// are any, so the swfnew character will get
		...
		di.m_character->set_const_cxform(color_xform != NULL ? color_xform : &cxform::identity);
		di.m_character->set_const_matrix(mat != NULL ? mat : &matrix::identity);
		di.m_character->set_ratio(ratio);
		di.m_character->set_clip_depth(clip_depth);
		di.m_character->set_const_effect(eff != NULL ? eff : &effect::identity);
		...
		stringi_pointer key(di.m_character->get_name().to_tu_stringi());
		if(key.m_str->length() > 0 && m_character_map.find(key) == m_character_map.end())
			m_character_map.add(key, di.m_character.get_ptr());
	...
	// the specified depth.
	{
		//IF_VERBOSE_DLIST(log_msg("dl::move(%d)\n", depth));
		...
		if (color_xform != NULL)
		{
			ch->set_const_cxform(color_xform);
		}
		if (mat != NULL)
		{
			ch->set_const_matrix(mat);
		}
		if(eff != NULL)
		{
			ch->set_const_effect(eff);
		}
		ch->set_ratio(ratio);
	...
	void	display_list::replace_display_object(
		...
		float ratio,
		Uint16 clip_depth)
	// Puts a swfnew character at the specified depth, replacing any
	// existing character.  If use_cxform or use_matrix are false,
	// then keep those respective properties from the existing
	// character.
	{
		IF_VERBOSE_DLIST(const char* name = ch->get_name(); log_msg("dl::replace(%d, '%s')\n", depth, name));//xxxxx

		int	size = m_display_object_array.size();
		int	index = find_display_index(depth);
		if (index < 0 || index >= size)
		{
			add_display_object(ch, depth, true, color_xform, mat, eff, ratio, clip_depth);
			return;
		}
		
		display_object_info&	di = m_display_object_array[index];
		character* old_ch = di.m_character.get_ptr();
		matrix custom_matrix = old_ch->get_matrix();
		cxform custom_cxform = old_ch->get_cxform();
		effect custom_effect = old_ch->get_effect();
		const matrix* const_matrix = old_ch->m_custom && old_ch->m_matrix == &(old_ch->m_custom->m_matrix) ? &custom_matrix : old_ch->m_matrix;
		const cxform* const_cxform = old_ch->m_custom && old_ch->m_cxform == &(old_ch->m_custom->m_cxform) ? &custom_cxform : old_ch->m_cxform;
		const effect* const_effect = old_ch->m_custom && old_ch->m_effect == &(old_ch->m_custom->m_effect) ? &custom_effect : old_ch->m_effect;

		// Put the swfnew character in its place.
		...
		if (color_xform == NULL)
		{
			...
			if(const_cxform == &custom_cxform)
				ch->set_cxform(custom_cxform);
			else
				ch->set_const_cxform(const_cxform);
		}

		if (mat == NULL)
		{
			// Use the matrix from the old character.
			if(const_matrix == &custom_matrix)
				ch->set_matrix(custom_matrix);
			else
				ch->set_const_matrix(const_matrix);
		}

		if (eff == NULL)
		{
			// Use the effect from the old character.
			if(const_effect == &custom_effect)
				ch->set_effect(custom_effect);
			else
				ch->set_const_effect(const_effect);
		}
	}
	...
	void display_list::construct()
	{
		// That there was no crash because of actions script
		// we iterate through the copy
		if(m_display_object_array.size() > 0)
		{
			// push dlist into array
			array<smart_ptr<character> >& stack = m_display_object_array[0].m_character->get_player()->m_dlist_stack;
			int initial_size = stack.size();
			int n = m_display_object_array.size();
			for (int i = n - 1; i >= 0; i--)
			{
				stack.push_back(m_display_object_array[i].m_character.get_ptr());
			}
			
			for (int i = 0; i < n; i++)
			{
				character*	ch = stack.back().get_ptr();
				if (ch)
				{
					ch->construct();
				}
	...
	bool display_list::advance(float delta_time)
	// advance referenced characters.
	{
		...
		if(m_display_object_array.size() > 0)
		{
			...
			{
				character*	ch = stack.back().get_ptr();
				if (ch && ch->m_need_advance)
				{
					ch->advance(delta_time);
					need_advance |= ch->m_need_advance;
				}
				stack.pop_back();
			}
	...
	// are obscured by higher depths.
	{
		...
		gameswf::blend_mode::id previous_blend_mode = gameswf::blend_mode::NORMAL;
		...
		for (int i = 0; i < m_display_object_array.size(); i++)
		{
			...
			const cxform& cx = ch->get_cxform();
			if(cx.m_[3][0] == 0.0f && cx.m_[3][1] == 0.0f)
			{
				// Alpha is 0, nothing will be displayed
				continue;
			}
			...
			{
				previous_blend_mode = ch->m_player->m_blend_stack.size() > 0 ? ch->m_player->m_blend_stack.back() : gameswf::blend_mode::NORMAL;
...
/*			if (masked)
			...
			if (ch->get_clip_depth() > 0)
			{
				...
				// restore current blend mode
				if(previous_blend_mode != gameswf::blend_mode::NORMAL)
					render::set_blend_mode(previous_blend_mode);
			}
		}
