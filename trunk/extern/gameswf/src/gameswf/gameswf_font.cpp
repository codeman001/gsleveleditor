...
namespace gameswf
{
		...
		if (tag_type == 10)
		{
			...
			if (m->get_create_font_shapes() == DO_LOAD_FONT_SHAPES)
			{
				...
				{for (int i = 0; i < count; i++)
				{
					...
					shape_character_def* s = swfnew shape_character_def(m->get_player());
					s->read(in, 2, false, m);
	...
	{
		// first try bitmap font
		bitmap_glyph_provider* bgp = get_player()->m_context->m_bitmap_glyph_provider;
		if (bgp)
		{
			bitmap_font_entity* bfe = bgp->get_font_entity(m_fontname, m_is_bold, m_is_italic);
			if(bfe)
				return bfe->get_height();
		}

		// second try font device
		glyph_provider* fp = get_player()->m_context->m_glyph_provider;
		if (fp)
		{
			face_entity* fe = fp->get_face_entity(m_fontname, m_is_bold, m_is_italic);
			if(fe)
				return (float)(-fe->m_face->descender + fe->m_face->ascender);
		}

		return 0.0f;
	}

	float	font::get_units_per_em() const
	{
		// first try bitmap font
		bitmap_glyph_provider* bgp = get_player()->m_context->m_bitmap_glyph_provider;
		if (bgp)
		...
		if (fp)
		{
			face_entity* fe = fp->get_face_entity(m_fontname, m_is_bold, m_is_italic);
			if(fe)
				return fe->m_face->units_per_EM;
		}

		return 1.0f;
	}

	void	font::copy_from(font* f)
	{
		m_fontname = f->m_fontname;
		m_unicode_chars = f->m_unicode_chars;
		m_shift_jis_chars = f->m_shift_jis_chars;
		m_ansi_chars = f->m_ansi_chars;
		m_is_italic = f->m_is_italic;
		m_is_bold = f->m_is_bold;
		m_wide_codes = f->m_wide_codes;
		m_ascent = f->m_ascent;
		m_descent = f->m_descent;
		m_leading = f->m_leading;
		m_table_hint = f->m_table_hint;
	}
	...
	bool	font::get_glyph(glyph* g, Uint16 code, int fontsize) const
	{
		...
		// first try bitmap font provider
		bitmap_glyph_provider* bgp = get_player()->m_context->m_bitmap_glyph_provider;
		if (bgp)
		{
			g->m_type = glyph::GLYPH;
			bitmap_font_entity* bfe = bgp->get_font_entity(m_fontname, m_is_bold, m_is_italic);
			g->m_fe = (gameswf::face_entity*)bfe;
			if(bfe)
			{
				g->m_fontlib_glyph = bfe->get_char_image(code, fontsize, &g->m_bounds, &g->m_glyph_advance);
		...
		if (fp)
		{
			...
			g->m_fe = fp->get_face_entity(m_fontname, m_is_bold, m_is_italic);
			if (g->m_fontlib_glyph != NULL)
			{
				if (is_define_font3())
				{
