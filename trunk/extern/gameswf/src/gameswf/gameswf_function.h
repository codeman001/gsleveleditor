...
namespace gameswf
{
	...
	struct as_function : public as_object
	{
		...
		virtual const char*	_typeof() { return "function"; }
		...
		virtual bool get_member(const tu_stringi& name, as_value* val)
		{
			if (name == "prototype")
			{
				val->set_as_object(m_prototype.get_ptr());
				return true;
			}
		...
		virtual void clear_refs(hash<as_object*, bool>* visited_objects, as_object* this_ptr)
		{
			...
			if(m_prototype.get_ptr() != NULL)
			{
				if (m_prototype.get_ptr() == this_ptr)
					m_prototype = NULL;
				else
					m_prototype->clear_refs(visited_objects, this_ptr);
			}
		}
		...
		virtual void this_alive()
		{
			...
			if(m_prototype.get_ptr() != NULL)
				m_prototype->this_alive();
		}
		
		smart_ptr<as_object> m_prototype;
	};
	...
	struct as_s_function : public as_function
	{
		...
		virtual bool get_member(const tu_stringi& name, as_value* val)
		{
			if (get_builtin(BUILTIN_S_FUNCTION_METHOD, name, val))
				return true;
			
			return as_function::get_member(name, val);
		}
	};
