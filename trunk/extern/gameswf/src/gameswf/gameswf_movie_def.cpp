...
namespace gameswf
{
	...
	{
		s_tag_loaders.set_capacity(96);

		return s_tag_loaders.size() > 0;
	}
	...
	movie_def_impl::movie_def_impl(player* player, 
		...
		m_origin_in(NULL),
		m_thread(NULL),
		m_has_init_actions(false),
		m_file_id(0)
	{
		m_file_id = player->m_context->create_file_id();
		memset(m_debug_id, 0, sizeof(m_debug_id));
	}
	...
	movie_def_impl::~movie_def_impl()
	{
		...
		{for (int i = 0, n = m_init_action_list.size(); i < n; i++)
		{
			...
			{
				swfdelete (m_init_action_list[i][j]);
			}
		}}
	...
	bool	movie_def_impl::get_labeled_frame(const tu_stringi& label, int* frame_number)
	...
	// kept in this object.
	{
		stringi_pointer n(get_player()->get_const_string(name).to_tu_stringi());
		...
		// Read a .SWF movie.
	{
		...
		{
			in = get_buffered_file(in);
			m_zlib_in = in;
		}

		m_str = swfnew stream(in, compressed);
		...
		//m_thread = swfnew tu_thread(movie_def_loader, this);
