...
namespace gameswf
{
	...
	{
		fn.result->set_as_object(swfnew as_object(fn.get_player()));
	}
...
//		assert(player);
	...
	bool	as_object::set_member(const tu_stringi& _name, const as_value& new_val)
	{
		const tu_stringi& name = _name.is_permanent() ? _name : (m_player != NULL ? m_player->get_const_string(_name.to_tu_string()).to_tu_stringi() : get_static_string(_name.to_tu_string()).to_tu_stringi());

//		printf("SET MEMBER: %s at %p for object %p\n", name.c_str(), val.to_object(), this);

		as_member_hash::iterator it = this->m_members.find(name);
		const bool exists = it != this->m_members.end();
		if (exists)
		{
			if (it->second.is_property())
			{
				as_value old_val;
				if(get_member(name, &old_val))
					old_val.set_property(new_val);
				return true;
			}
		}
		...
		if (m_watch)
		{
			as_value old_val = exists ? it->second : as_value();
			...
			if (watch.m_func)
			{
				...
				val.set_undefined();
				(*watch.m_func)(fn_call(&val, this, &env, 4, env.get_top_index(), name.c_str()));
			}
		}

		notify_set_member(name, new_val);
	...
	bool	as_object::set_member(int index, const as_value& val)
	{
		assert(false);
		return false;
	}
	
	bool	as_object::get_member(int index, as_value* val)
	{
		assert(false);
		return false;
	}
	...
	void	as_object::clear_refs(hash<as_object*, bool>* visited_objects, as_object* this_ptr)
	{
		...
		as_value undefined;
		for (as_member_hash::iterator it = m_members.begin();
			it != m_members.end(); ++it)
		{
		...
		if(m_proto.get_ptr() != NULL)
		{
			if (m_proto.get_ptr() == this_ptr)
				m_proto = NULL;
			else
				m_proto->clear_refs(visited_objects, this_ptr);
		}

		if(m_ctor.to_object() != NULL)
		{
			if (m_ctor.to_object() == this_ptr)
				m_ctor.set_undefined();
			else
				m_ctor.to_object()->clear_refs(visited_objects, this_ptr);
		}
	}
	...
	{
		as_member_hash::const_iterator it = m_members.begin();
		while (it != m_members.end())
		{
			if (it->second.is_enum())
			{
				env->push(it->first.to_tu_string());
...
//		}
		...
		const as_value& user_data)
	{
		...
		if (m_watch == NULL)
		{
			m_watch = swfnew stringi_hash<as_watch>;
		}
	...
	// Copy all members from 'this' to target
	{
		if (target)
		{
			for (as_member_hash::const_iterator it = m_members.begin(); 
				it != m_members.end(); ++it ) 
			{ 
				target->set_member(it->first.to_tu_stringi(), it->second); 
			} 
		}
	}
	...
	// Find the object referenced by the given target.
	{
		...
		{
			return find_target(target.to_tu_string().c_str());
		}
	}

	exported_module as_object* as_object::find_target(const char* target)
	{
		...
		if (*path == '/')
		{
			return m_player->get_root()->get_root_movie()->find_target(path + 1);
		}
		...
		if (slash)
		{
			tu_string name(path, int(slash - path));
			get_member(name.to_tu_stringi(), &val);
			tar = val.to_object();
			if (tar)	
			{
		...
		if (tar == NULL)
		{
			IF_VERBOSE_ACTION(log_error("can't find target %s\n", path));
		}
		return tar;
	}
	...
	void as_object::this_alive()
	{
		...
		if (m_player != NULL && m_player->is_garbage(this))
		{
			...
			if(m_ctor.is_object() && m_ctor.to_object() != NULL)
				m_ctor.to_object()->this_alive();
		}
	}
	...
	{ 	 
		m_proto = swfnew as_object(get_player()); 	 
		...
		{ 	 
			...
			//	printf("create_proto %s\n", ch->m_name.c_str());
			//}

			// backup current values
			as_member_hash old_members = m_members;

			// copy values from prototype
			prototype->copy_to(this); 	 

			// apply old values to properties
			for (as_member_hash::const_iterator it = old_members.begin(); 
				it != old_members.end(); ++it ) 
			{
				as_value new_value;
				const tu_stringi& name = it->first.to_tu_string().to_tu_stringi();
				if(get_member(name, &new_value))
				{
					// new value is property? if so, apply old value to property
					if(new_value.is_property() && !it->second.is_property())
					{
						new_value.set_property(it->second);
					}
					else if(new_value != it->second)
					{
						// printf("   replacing %s value from %s to %s\n", name.c_str(), new_value.to_string(), it->second.to_string());
						set_member(name, it->second);
					}
				}
			}
