...
namespace gameswf
{
	...
	// For holding a pre-tesselated shape.
	{
		...
		void set_triangles(const coord_component* pts, int pts_count, const unsigned short* indexes, int index_count);
	...
	private:
		...
		array<unsigned short> m_index_list;
	};
	...
	// information on fill and line styles.
	{
		...
		character* get_dead_instance(character* parent, int id);
