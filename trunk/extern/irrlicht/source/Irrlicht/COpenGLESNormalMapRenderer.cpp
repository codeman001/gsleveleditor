...
//! Constructor
COpenGLESNormalMapRenderer::COpenGLESNormalMapRenderer(video::COpenGLDriver* driver, 
	...
	: COpenGLShaderMaterialRenderer(driver, 0, baseMaterial), CompiledShaders(true)
{
	...
	if (renderer)
	{
		...
		video::COpenGLESNormalMapRenderer* nmr = reinterpret_cast<video::COpenGLESNormalMapRenderer*>(renderer);
...
//! Destructor
COpenGLESNormalMapRenderer::~COpenGLESNormalMapRenderer()
{
	if (CallBack == this)
		CallBack = 0;
...
s32 COpenGLESNormalMapRenderer::getRenderCapability() const
{
	if (Driver->queryFeature(video::EVDF_ARB_FRAGMENT_PROGRAM_1) &&
		Driver->queryFeature(video::EVDF_ARB_VERTEX_PROGRAM_1))
		return 0;
...
void COpenGLESNormalMapRenderer::OnSetConstants(IMaterialRendererServices* services, s32 userData)
{
