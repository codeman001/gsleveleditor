...
void CViewWindow::createIrrDevice()
{
	...
	// add box
	scene::ISceneNode *pBox = m_smgr->addCubeSceneNode(40);
	pBox->setPosition( core::vector3df(0,0,0) );
	pBox->setMaterialTexture( 0, m_driver->getTexture("media/stones.jpg") );
	pBox->setMaterialFlag( video::EMF_LIGHTING, false );
#endif
...
{		
	...
	m_driver->draw3DLine
		(
			...
			core::vector3df( 400.0f, 0.0f, 0.0f ), 
			SColor(255,255,0,0)
		);
	// y
	m_driver->draw3DLine
		(
			core::vector3df( 0.0f, 0.0f, 0.0f ),
			core::vector3df( 0.0f, 400.0f, 0.0f ), 
			SColor(255,0,255,0)
		);
	// z
	m_driver->draw3DLine
		(
			core::vector3df( 0.0f, 0.0f, 0.0f ),
			core::vector3df( 0.0f, 0.0f, 400.0f ), 
			SColor(255,0,0,255)
		);

	gui::IGUIFont* font = m_device->getGUIEnvironment()->getBuiltInFont();	
	if ( font )
	{
		int x, y;		
		if ( getScreenCoordinatesFrom3DPosition( core::vector3df( 400.0f, 0.0f, 0.0f ), &x, &y ) )
			font->draw(L"X", core::rect<s32>(x, y, 40,20), video::SColor(255,255,0,0));

		if ( getScreenCoordinatesFrom3DPosition( core::vector3df( 0.0f, 400.0f, 0.0f ), &x, &y ) )
			font->draw(L"Y", core::rect<s32>(x, y, 40,20), video::SColor(255,0,255,0));

		if ( getScreenCoordinatesFrom3DPosition( core::vector3df( 0.0f, 0.0f, 400.0f ), &x, &y ) )
			font->draw(L"Z", core::rect<s32>(x, y, 40,20), video::SColor(255,0,0,255));
	}
	// get line ray
	scene::ISceneCollisionManager* collMan = m_smgr->getSceneCollisionManager();
	
	core::vector2di point = m_device->getCursorControl()->getPosition();
	core::line3d<f32> ray = collMan->getRayFromScreenCoordinates( point, m_smgr->getActiveCamera() );
	...
	{
		int x, y;		
		if ( getScreenCoordinatesFrom3DPosition( hit, &x, &y ) )
		{
			wchar_t lpString[64] ={0};
			swprintf( lpString, 64, L"%f %f %f", hit.X, hit.Y, hit.Z );
			font->draw( lpString, core::rect<s32>(x, y, 60,20), video::SColor(255,255,255,255));
		}
	}
